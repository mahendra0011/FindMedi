...
function getAppointmentModeMeta(appt: DisplayAppointment) {
...
const StatusBadge = ({ status, mapping }: { status: string; mapping?: Record<string, string> }) => {
      ...
      try {
        ...
        if (lbRaw?.bookings?.length) {
          setTestBookings(lbRaw.bookings.map(b => ({
            ...
            date: String(b.bookingDate || b.date || new Date().toISOString()),
            timeSlot: String(b.timeSlot || ''),
            amount: Number(b.totalAmount || b.discountedAmount || 0),
            visitType: b.homeCollectionAddress ? 'Home Collection' : String(b.visitType || ''),
            reportsAvailable: ['Completed', 'Report Ready', 'Delivered'].includes(String(b.status)),
          })));
        }
        ...
        <div className="relative z-10 flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm text-center min-w-[80px]">
              ...
              <motion.p key={tipIndex} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="font-medium text-xs max-w-[200px] leading-tight">
                {healthTips[tipIndex]}
              </motion.p>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
            ...
            return (
              <div className="space-y-3">
                {list.slice(0, 5).map(a => {
                  ...
                  const docName = a.doctor || a.doctorName || (typeof a.doctorId === 'object' ? a.doctorId?.name : a.doctorId) || 'Doctor';
                  return (
                    <div key={a._id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-3.5 bg-muted/20 rounded-2xl border border-border/30 hover:bg-muted/40 hover:border-primary/20 transition-all duration-200 gap-3">
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
                        ...
                        {apptTab === 'upcoming' && (
                          <div className="flex items-center gap-2">
                            {mode.key === 'home' && (
                              <Button size="sm" variant="outline" className="text-xs h-8 rounded-xl border-violet-500/30 text-violet-600 hover:bg-violet-500/10" onClick={() => router.push('/patient/home-visit')}>
                                <MapPin className="w-3 h-3 mr-1" /> Track Route
                              </Button>
                            )}
                            {mode.key === 'video' && (
                              <Button size="sm" variant="outline" className="text-xs h-8 rounded-xl border-cyan-500/30 text-cyan-600 hover:bg-cyan-500/10" onClick={() => router.push('/patient/video-calls')}>
                        ...
                        {apptTab === 'today' && (
                          <div className="flex items-center gap-2">
                            ...
                            {mode.key === 'video' && (
                              <Button size="sm" className="text-xs h-8 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white" onClick={() => router.push('/patient/video-calls')}>
                                <Video className="w-3 h-3 mr-1" /> Start Video
                              </Button>
                            )}
                            {mode.key === 'home' && (
                              <Button size="sm" className="text-xs h-8 rounded-xl bg-violet-600 hover:bg-violet-700 text-white" onClick={() => router.push('/patient/home-visit')}>
                        ...
                        {apptTab === 'complete' && (
                          <Button size="sm" variant="outline" className="text-xs h-8 rounded-xl text-primary border-primary/20 hover:bg-primary/5" onClick={() => router.push('/doctors')}>
                            Book Again
                          </Button>
                        )}
                      </div>
        ...
        <div className="bg-card rounded-3xl border border-border/50 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <Button variant="outline" size="sm" className="gap-1.5 rounded-xl text-primary border-primary/20 hover:bg-primary/5 hover:text-primary" onClick={() => router.push('/patient/history')}>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-card rounded-3xl border border-border/50 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <Button variant="outline" size="sm" className="gap-1.5 rounded-xl text-cyan-500 border-cyan-500/20 hover:bg-cyan-500/5 hover:text-cyan-500" onClick={() => router.push('/patient/bookings')}>
          ...
          {recentTests.length > 0 ? (
              ...
              {recentTests.map(t => (
                <div key={t._id} className="group flex items-center justify-between p-3.5 bg-muted/20 rounded-2xl border border-border/30 hover:bg-muted/40 hover:border-cyan-500/20 transition-all duration-200">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5 ml-4">{(t.tests || []).slice(0, 2).join(', ')}{t.tests && t.tests.length > 2 ? ` +${t.tests.length - 2}` : ''}</p>
                  </div>
            ...
            <div className="text-center py-10">
              ...
              <Button size="sm" className="mt-4 rounded-xl shadow-lg shadow-cyan-500/20 bg-gradient-to-r from-cyan-500 to-cyan-600 text-white hover:from-cyan-600 hover:to-cyan-700" onClick={() => router.push('/patient/services')}>
        ...
        <div className="bg-card rounded-3xl border border-border/50 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <Button variant="outline" size="sm" className="gap-1.5 rounded-xl text-amber-500 border-amber-500/20 hover:bg-amber-500/5 hover:text-amber-500" onClick={() => router.push('/patient/medicine-orders')}>
              View <ChevronRight className="w-3.5 h-3.5" />
            </Button>
          </div>
          {recentOrders.length > 0 ? (
            ...
            <div className="text-center py-10">
              ...
              <Button size="sm" className="mt-4 rounded-xl shadow-lg shadow-amber-500/20 bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600" onClick={() => router.push('/pharmacy')}>
        ...
        <div className="bg-card rounded-3xl border border-border/50 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <Button variant="outline" size="sm" className="gap-1.5 rounded-xl text-blue-500 border-blue-500/20 hover:bg-blue-500/5 hover:text-blue-500" onClick={() => router.push('/patient/prescriptions')}>
              View <ChevronRight className="w-3.5 h-3.5" />
            </Button>
          </div>
