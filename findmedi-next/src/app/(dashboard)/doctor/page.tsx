...
import {
  ...
  Building2, Check, X, CalendarClock, LucideIcon
} from 'lucide-react';
...
interface PreConsultationDetails {
...
function getAppointmentModeMeta(appt: AppointmentItem | null | undefined): ModeMeta {
  if (!appt) {
...
export default function DoctorDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const apptsRaw = (a as { data?: AppointmentItem[] } | AppointmentItem[]) || [];
      const appts: AppointmentItem[] = Array.isArray(apptsRaw) ? apptsRaw : (apptsRaw?.data || []);
      const myAppointments = appts?.filter(apt => 
        apt.doctor?.toLowerCase() === docName
      ) || [];
      setAppointments(myAppointments);

      const reviewsRaw = (r as { data?: ReviewItem[] } | ReviewItem[]) || [];
      const reviewsArray: ReviewItem[] = Array.isArray(reviewsRaw) ? reviewsRaw : (reviewsRaw?.data || []);
      setReviews(reviewsArray?.filter(rv => rv.doctorName === user?.name) || []);
      
      const billsRaw = (b as { data?: BillItem[]; bills?: BillItem[] } | BillItem[]) || [];
      const billsArray: BillItem[] = Array.isArray(billsRaw) ? billsRaw : (billsRaw?.data || billsRaw?.bills || []);
      const myBills = billsArray?.filter(bill => 
        bill.doctor?.toLowerCase() === docName
      ) || [];
      setBills(myBills);

      const recordsRaw = (records as { data?: LabReportItem[]; records?: LabReportItem[] } | LabReportItem[]) || [];
      const allRecords: LabReportItem[] = Array.isArray(recordsRaw) ? recordsRaw : (recordsRaw?.data || recordsRaw?.records || []);
      ...
      const refundRaw = (rf as { payments?: RefundItem[]; data?: RefundItem[] } | RefundItem[]) || [];
      const refundArray: RefundItem[] = Array.isArray(refundRaw) ? refundRaw : (refundRaw?.payments || refundRaw?.data || []);
                ...
                <div key={rv._id} className="p-4 bg-muted/30 rounded-xl">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map(s => (
                        <Star key={s} className={`w-3.5 h-3.5 ${s <= (rv.rating || 0) ? 'text-warning fill-warning' : 'text-muted'}`} />
                      ))}
                    </div>
                  </div>
