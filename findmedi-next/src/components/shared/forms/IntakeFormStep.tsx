...
export interface IntakeFormData {
  ...
  pastMedicalHistory: { hasHistory: boolean | null; details: string };
  currentTreatment: {
    hasPastTreatment: boolean | null;
    doctorName?: string;
    cityState?: string;
    when?: string;
    prescriptionFile?: string;
    takingMedicines?: boolean | null;
  };
  testReports: { hasReports: boolean | null; reportFile?: string };
  currentMedications: { hasMedications: boolean | null; details: string };
  allergies: { hasAllergies: boolean | null; details: string };
...
export default function IntakeFormStep({ formData, setFormData }: IntakeFormStepProps) {
  const [uploadingPrescription, setUploadingPrescription] = useState(false);
  const [uploadingReport, setUploadingReport] = useState(false);
  const [uploadMenu, setUploadMenu] = useState<'prescription' | 'report' | null>(null);
  
  const [activeSection, setActiveSection] = useState<string | null>('chief_complaint');
  ...
  const sectionRefs: Record<string, React.RefObject<HTMLDivElement | null>> = {
    chief_complaint: useRef<HTMLDivElement>(null),
    past_history: useRef<HTMLDivElement>(null),
    current_treatment: useRef<HTMLDivElement>(null),
    test_reports: useRef<HTMLDivElement>(null),
    current_meds: useRef<HTMLDivElement>(null),
    allergies: useRef<HTMLDivElement>(null),
    family_history: useRef<HTMLDivElement>(null)
  };

  const handleUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
    setUploading: React.Dispatch<React.SetStateAction<boolean>>,
    fieldPath: 'prescriptionFile' | 'reportFile'
  ) => {
    ...
    try {
      const res = await api.uploadFile(file) as unknown as { url?: string };
      if (res && res.url) {
  ...
  const renderSection = (id: string, title: string, isCompleted: boolean, children: React.ReactNode) => {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${isCompleted ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400' : isActive ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted/60 text-muted-foreground'}`}>
            ...
            <div className="space-y-4 pt-3 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-2 pt-2 border-t border-border/40">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant={formData.currentTreatment.takingMedicines === false ? "default" : "outline"} size="sm" onClick={() => setFormData(p => ({ ...p, currentTreatment: { ...p.currentTreatment, takingMedicines: false } }))}>No</Button>
                </div>
              </div>
