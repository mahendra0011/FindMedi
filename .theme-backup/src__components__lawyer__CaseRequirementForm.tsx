...
export const CaseRequirementForm: React.FC<Props> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 flex items-center justify-between gap-4">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:text-slate-100 placeholder:text-slate-400"
          required
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            ...
            className={`p-3.5 rounded-2xl border text-left transition-all flex items-start gap-3 ${
              urgency === 'normal'
                ? 'border-indigo-600 bg-indigo-50/40 dark:bg-indigo-950/20 ring-1 ring-indigo-500'
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {documents.map((doc, idx) => (
              <Badge
                key={idx}
                variant="secondary"
                className="px-2.5 py-1 text-xs gap-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800"
              >
