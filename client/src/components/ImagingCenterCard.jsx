...
import {
  ...
  Eye, BadgeCheck, Zap, ArrowRight, ChevronRight, Sparkles,
  Calendar, Scan, Radio, Microscope
} from 'lucide-react';
...
export default function ImagingCenterCard({ clinic, index = 0 }) {
  const navigate = useNavigate();
  const {
    _id, name, logo, type = 'Imaging Center', rating = 4.5, reviewsCount = 200,
    verified = true, open = true, tags = [],
    testsAvailable = 120, reportTime = 'Within 24 hrs',
    distance = '2.5 km', phone = '9876543210', startingPrice = 500,
    imagingTypes = ['MRI', 'CT', 'X-Ray', 'Ultrasound'], emergencyImaging = false
  } = clinic || {};
    ...
    >
        ...
        {logo ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-primary/30 via-primary/15 to-primary/5 flex items-center justify-center">
            <div className="relative">
              <Scan className="w-16 h-16 text-primary/20" />
      ...
      <div className="p-4 space-y-3 flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap gap-1.5">
          {tags.includes('MRI Available') && (
            <TagBadge icon={Radio} variant="blue">
              MRI Available
            </TagBadge>
          )}
          {tags.includes('CT Available') && (
        ...
        <div className="bg-gradient-to-br from-muted/50 to-muted/5 rounded-xl border border-border/40 p-3.5 space-y-2.5">
          <InfoRow icon={Scan} label="Imaging Types" value={imagingTypes.join(', ')} />
          <Divider />
          <InfoRow icon={Clock} label="Report Time" value={reportTime} />
          <Divider />
          <div className="grid grid-cols-2 gap-2">
            <InfoRow icon={MapPin} label="Distance" value={distance} />
            <InfoRow icon={Microscope} label="Mode" value="Visit Required" />
          </div>
          <Divider />
        ...
        <div className="flex items-center justify-between p-3 rounded-xl bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border border-primary/15">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-sm text-muted-foreground line-through">₹{startingPrice + 300}</span>
        ...
        <div className="flex gap-2.5 pt-1 mt-auto">
          ...
          <Button
            ...
            onClick={() => navigate(`/imaging/${_id}/details`)}
          >
