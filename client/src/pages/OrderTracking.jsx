          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setShowReportModal(false)}>
            <div className="bg-card rounded-2xl border border-border/60 p-6 max-w-sm w-full shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-2 mb-4">
                <Button className="w-full rounded-xl gap-2" onClick={() => { const base = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'; window.open(base + '/records/' + (orderData?._id || '') + '/prescription-pdf', '_blank'); setShowReportModal(false); }}>
                  <FileText className="w-4 h-4" /> Download PDF
                </Button>
                <Button variant="outline" className="w-full rounded-xl gap-2" onClick={() => { const url = window.location.href; if (navigator.share) { navigator.share({ title: 'Medical Report', url }); } else { navigator.clipboard?.writeText(url); toast.success('Link copied!'); } setShowReportModal(false); }}>
