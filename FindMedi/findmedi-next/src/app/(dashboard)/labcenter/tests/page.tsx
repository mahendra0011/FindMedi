/**
 * Test Catalog — ported from client/src/pages/labcenter/LabTestCatalog.jsx (Phase 4).
 * Browsable lab test catalog with category filters.
 */
'use client';
  ...
  const { data: testsData, isLoading: loading } = useQuery({
    ...
    queryFn: async (): Promise<CatalogTest[]> => {
      ...
      return (Array.isArray(res) ? res : []).map((t, i) => ({
        ...
        reportTime: typeof t.reportTime === 'string' ? t.reportTime : undefined,
        prescriptionReq: Boolean(t.prescriptionReq),
        homeCollection: Boolean(t.homeCollection),
      }));
    },
    staleTime: 300_000,
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">{filtered.length} tests</p>
        </div>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {['All', ...categories].map((c) => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${catFilter === c ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}
            >
              {c}
            </button>
          ))}
                  ...
                  <div>
                    ...
                    {(test.discount ?? 0) > 0 && (
                      <p className="text-xs text-success">
                        {test.discount}% off • ₹{test.price - (test.price * (test.discount ?? 0)) / 100}
                      </p>
                    )}
                  </div>
