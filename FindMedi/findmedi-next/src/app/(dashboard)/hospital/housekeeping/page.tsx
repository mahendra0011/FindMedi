/**
 * Housekeeping — ported from client/src/pages/Housekeeping.jsx (Phase 4).
 * Cleaning tasks, checklists, infection protocol, supervisor verification.
 */
'use client';

import { useState } from 'react';
import { Search, Plus, Clock, X, CheckCircle, AlertTriangle, Home, Sparkles, Shield, ClipboardList, Bell } from 'lucide-react';
...
import type { HousekeepingTask, HousekeepingStats } from '@/features/housekeeping/types';
...
const emptyStats: HousekeepingStats = { total: 0, pending: 0, inProgress: 0, completed: 0 };
...
export default function HousekeepingPage() {
  ...
  const { data: tasksData, isLoading } = useHousekeepingTasks(search, statusFilter);
  const { data: statsData } = useHousekeepingStats();

  const createMut = useCreateTask();
  const completeMut = useCompleteTask();
  const verifyMut = useVerifyTask();
  const autoCreateMut = useAutoCreateOnDischarge();

  const tasks = ((tasksData ?? []) as unknown as HousekeepingTask[]).filter(
    (t) =>
      (t.room ?? '').toLowerCase().includes(search.toLowerCase()) ||
      (t.ward ?? '').toLowerCase().includes(search.toLowerCase()),
  );
  const stats: HousekeepingStats = { ...emptyStats, ...((statsData as Partial<HousekeepingStats> | undefined) ?? {}) };
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { l: 'Total Tasks', v: stats.total, c: 'text-foreground', ic: Home },
          { l: 'Pending', v: stats.pending, c: 'text-warning', ic: Clock },
          { l: 'In Progress', v: stats.inProgress, c: 'text-primary', ic: Sparkles },
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search rooms, wards..."
            className="pl-10"
            value={search}
        ...
        <Button onClick={() => autoCreateMut.mutate({ patientId: 'demo' })} variant="outline">
          <Bell className="w-4 h-4 mr-1" /> Simulate Discharge Alert
        </Button>
      </div>

      <div className="space-y-4">
        {tasks.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-10">No housekeeping tasks found.</p>
        ) : (
          tasks.map((task) => {
            ...
            return (
              <div
                key={task._id}
                className={`bg-card rounded-xl border shadow-sm ${task.isInfectionCase ? 'border-destructive/50 bg-destructive/5' : ''}`}
              >
                <div className="p-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : task._id)}>
                  <div className="flex items-center gap-3">
                    <Home className="w-5 h-5 text-primary" />
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${statusColors[task.status] ?? ''}`}>
                      {task.status}
                    </span>
                    {task.isInfectionCase && <AlertTriangle className="w-4 h-4 text-destructive" />}
                    <div className="flex-1">
                      <p className="font-medium text-foreground">
                        {task.room} · Bed {task.bedNumber}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {task.ward} · {task.type} · {task.assignedTo || 'Unassigned'}
                      </p>
                    </div>
                  ...
                  <div className="px-4 pb-4 border-t pt-3 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{task.assignedTo || 'N/A'}</p>
                      </div>
                      {task.notes && (
                    ...
                    {task.isInfectionCase && (
                      <div className="bg-destructive/5 rounded-lg p-3 text-xs text-destructive">
                        ...
                        <strong>Infection Protocol:</strong> Terminal cleaning required. Use PPE, fumigation, and specialized
                        disinfectants.
                      ...
                      <div className="bg-warning/5 rounded-lg p-3">
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <div className="flex gap-2">
                              {['Excellent', 'Good', 'Fair', 'Poor'].map((r) => (
                                <button
                                  key={r}
                                  onClick={() => setVerifyData((d) => ({ ...d, rating: r }))}
                          ...
                          <div>
                            ...
                            <textarea
                              value={verifyData.comments}
                              onChange={(e) => setVerifyData((d) => ({ ...d, comments: e.target.value }))}
                              className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm resize-none h-16"
                            />
                          </div>
                          <div>
                            <label className="text-xs text-muted-foreground mb-1 block">Issues Found (if any)</label>
                            <textarea
                              value={verifyData.issuesFound}
                              onChange={(e) => setVerifyData((d) => ({ ...d, issuesFound: e.target.value }))}
                              className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm resize-none h-12"
                            />
                          </div>
                          <Button size="sm" onClick={() => verifyMut.mutate({ id: task._id, ...verifyData })}>
                            Verify & Approve
                          </Button>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">New Housekeeping Task</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input value={newTask.room} onChange={(e) => setNewTask({ ...newTask, room: e.target.value })} />
                </div>
                <div>
                  <label className="text-sm font-medium mb-1 block">Bed Number</label>
                  <Input value={newTask.bedNumber} onChange={(e) => setNewTask({ ...newTask, bedNumber: e.target.value })} />
                </div>
                <div>
                  <label className="text-sm font-medium mb-1 block">Ward</label>
                  <Input value={newTask.ward} onChange={(e) => setNewTask({ ...newTask, ward: e.target.value })} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    {['Routine Cleaning', 'Deep Cleaning', 'Discharge Cleaning', 'Terminal Cleaning', 'Fumigation'].map((t) => (
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={newTask.isInfectionCase}
                  onChange={(e) => setNewTask({ ...newTask, isInfectionCase: e.target.checked })}
                  className="w-4 h-4"
                />
              ...
              <Button
                className="w-full"
                onClick={() => createMut.mutate({ ...newTask }, { onSuccess: () => setShowCreate(false) })}
                disabled={createMut.isPending || !newTask.room}
              >
                Create Task
              </Button>
            </div>
