/**
 * Staff Management — ported from client/src/pages/Staff.jsx (Phase 4).
 * Directory, attendance, leave, shifts, training records.
 */
'use client';

import { useState } from 'react';
import { Search, Plus, Clock, X, Users, CheckCircle, User, Calendar, Activity, BookOpen, Award } from 'lucide-react';
...
import type { StaffMember, StaffStats } from '@/features/staff/types';
...
const departments = ['Cardiology', 'Neurology', 'Orthopedics', 'Pediatrics', 'Emergency', 'ICU', 'Lab', 'Pharmacy', 'hospital_admin'];

const emptyStats: StaffStats = { total: 0, active: 0, onDuty: 0, onLeave: 0 };
...
export default function StaffPage() {
  ...
  const [showAttendance, setShowAttendance] = useState<StaffMember | null>(null);
  const [showLeave, setShowLeave] = useState<StaffMember | null>(null);
  const [showShift, setShowShift] = useState<StaffMember | null>(null);
  const [showTraining, setShowTraining] = useState<StaffMember | null>(null);
  ...
  const { data: staffData, isLoading } = useStaffList(search, deptFilter);
  const { data: statsData } = useStaffStats();

  const createMut = useCreateStaff();
  const attendanceMut = useMarkAttendance();
  const leaveMut = useApplyLeave();
  const shiftMut = useAssignShift();
  const trainingMut = useAddTraining();

  const staff: StaffMember[] = staffData?.staff ?? [];
  const stats: StaffStats = { ...emptyStats, ...((statsData as Partial<StaffStats> | undefined) ?? {}) };
    ...
    <div className="container mx-auto py-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          {stats.active} active · {stats.onDuty} on duty · {stats.onLeave} on leave
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { l: 'Total Staff', v: stats.total, c: 'text-foreground', ic: Users },
          { l: 'Active', v: stats.active, c: 'text-success', ic: CheckCircle },
          { l: 'On Duty', v: stats.onDuty, c: 'text-primary', ic: Activity },
          { l: 'On Leave', v: stats.onLeave, c: 'text-warning', ic: Clock },
        ].map((s) => (
      ...
      <div className="space-y-4">
        {staff.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-10">No staff members found.</p>
        ) : (
          staff.map((s) => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {s.department} · {s.employeeId || 'No ID'} · {s.shift || 'No shift'}
                      </p>
                    </div>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${s.status === 'Active' ? 'bg-success/10 text-success' : s.status === 'On Leave' ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'}`}
                    >
                      {s.status}
                    </span>
                  </div>
                  ...
                  <div className="px-4 pb-4 border-t pt-3 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{s.joiningDate ? new Date(s.joiningDate).toLocaleDateString() : 'N/A'}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Salary</span>
                        <p className="font-medium">{s.salary ? `₹${s.salary}` : 'N/A'}</p>
                      </div>
                    </div>

                    {(s.attendance?.length ?? 0) > 0 && (
                      <div className="bg-muted/20 rounded-lg p-3">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {s.attendance?.slice(-7).map((a, i) => (
                            <div
                              key={i}
                              className={`text-[10px] px-2 py-1 rounded ${a.status === 'Present' ? 'bg-success/10 text-success' : a.status === 'Late' ? 'bg-warning/10 text-warning' : 'bg-destructive/10 text-destructive'}`}
                            >
                      ...
                      <div className="space-y-1">
                        ...
                        {s.leaves?.slice(-2).map((l, i) => (
                          <div key={i} className="text-xs bg-muted/30 rounded p-2">
                            {l.type} · {l.startDate ? new Date(l.startDate).toLocaleDateString() : ''} -{' '}
                      ...
                      <div className="space-y-1">
                        ...
                        {s.trainings?.map((t, i) => (
                          <div key={i} className="text-xs bg-muted/30 rounded p-2 flex items-center gap-2">
                            ...
                            {t.expiryDate && (
                              <span className="text-muted-foreground">(Exp: {new Date(t.expiryDate).toLocaleDateString()})</span>
                            )}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => {
                          setShiftData({ shift: 'Morning (6AM-2PM)', department: s.department ?? '', startDate: '' });
                          setShowShift(s);
                        }}
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => {
                          setTrainingData({ course: '', date: '', expiryDate: '', provider: '' });
                          setShowTraining(s);
                        }}
                      >
      ...
      {showAttendance && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAttendance(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-sm w-full p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  ...
                  onClick={() =>
                    attendanceMut.mutate(
                      { id: showAttendance._id, status, date: getISTDateString() },
                      { onSuccess: () => setShowAttendance(null) },
                    )
                  }
            ...
            <Button variant="outline" className="w-full" onClick={() => setShowAttendance(null)}>
              Cancel
            </Button>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setLeaveData((d) => ({ ...d, type: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="date" value={leaveData.startDate} onChange={(e) => setLeaveData((d) => ({ ...d, startDate: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">End Date</label>
                  <Input type="date" value={leaveData.endDate} onChange={(e) => setLeaveData((d) => ({ ...d, endDate: e.target.value }))} />
                </div>
              </div>
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setLeaveData((d) => ({ ...d, reason: e.target.value }))}
              ...
              <Button
                className="w-full"
                onClick={() =>
                  leaveMut.mutate({ id: showLeave._id, ...leaveData }, { onSuccess: () => setShowLeave(null) })
                }
              >
                Apply Leave
              </Button>
      ...
      {showShift && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowShift(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Assign Shift</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setShiftData((d) => ({ ...d, shift: e.target.value }))}
                  className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                >
                  {shifts.map((s) => (
              ...
              <div>
                ...
                <Input value={shiftData.department} onChange={(e) => setShiftData((d) => ({ ...d, department: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Start Date</label>
                <Input type="date" value={shiftData.startDate} onChange={(e) => setShiftData((d) => ({ ...d, startDate: e.target.value }))} />
              </div>
              <Button
                className="w-full"
                onClick={() =>
                  shiftMut.mutate({ id: showShift._id, ...shiftData }, { onSuccess: () => setShowShift(null) })
                }
              >
                Assign Shift
              </Button>
      ...
      {showTraining && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowTraining(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Add Training Record</h3>
            <p className="text-sm text-muted-foreground mb-4">{showTraining.name}</p>
            <div className="space-y-4">
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Course/Training</label>
                <Input
                  value={trainingData.course}
                  onChange={(e) => setTrainingData((d) => ({ ...d, course: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="date" value={trainingData.date} onChange={(e) => setTrainingData((d) => ({ ...d, date: e.target.value }))} />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setTrainingData((d) => ({ ...d, expiryDate: e.target.value }))}
                  />
                </div>
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Provider</label>
                <Input value={trainingData.provider} onChange={(e) => setTrainingData((d) => ({ ...d, provider: e.target.value }))} />
              </div>
              <Button
                className="w-full"
                onClick={() =>
                  trainingMut.mutate({ id: showTraining._id, ...trainingData }, { onSuccess: () => setShowTraining(null) })
                }
              >
                Save Training
              </Button>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={newStaff.employeeId} onChange={(e) => setNewStaff({ ...newStaff, employeeId: e.target.value })} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                  >
                    {['Doctor', 'Nurse', 'hospital_admin', 'Technician', 'Pharmacist', 'Lab Staff', 'Housekeeping', 'Other'].map((r) => (
                ...
                <div>
                  ...
                  <Input type="number" value={newStaff.salary} onChange={(e) => setNewStaff({ ...newStaff, salary: e.target.value })} />
                </div>
              </div>
              <Button
                className="w-full"
                onClick={() => createMut.mutate({ ...newStaff }, { onSuccess: () => setShowCreate(false) })}
                disabled={createMut.isPending || !newStaff.name}
              >
                Add Staff
              </Button>
            </div>
