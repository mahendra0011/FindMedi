...
import { useDeliveryProfile, useDeliveryReviews } from '@/features/delivery';

export default function DeliveryReviewsPage() {
  const { data: profile } = useDeliveryProfile();
  const { data: reviewsRaw, isLoading } = useDeliveryReviews();
  const [filter, setFilter] = useState<'all' | 'positive' | 'negative'>('all');

  const profileId = (profile as unknown as { _id?: string })?._id;

  const reviews = useMemo(() => {
    const all = reviewsRaw ?? [];
    if (!profileId) return all;
    // Filter to current delivery partner if IDs available
    const filtered = all.filter((r) => r.deliveryPartnerId === profileId || r.targetId === profileId);
    return filtered.length > 0 ? filtered : all;
  }, [reviewsRaw, profileId]);

  const filtered =
    filter === 'all'
      ? reviews
      : filter === 'positive'
        ? reviews.filter((r) => (r.rating ?? 0) >= 4)
        : reviews.filter((r) => (r.rating ?? 0) >= 1 && (r.rating ?? 0) <= 3);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl border p-6 text-center">
          <p className="text-5xl font-bold text-foreground">{avgRating}</p>
          <div className="flex justify-center gap-0.5 mt-2">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                key={s}
                className={`w-5 h-5 ${s <= Math.round(Number(avgRating)) ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30'}`}
              />
            ))}
          </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          { key: 'negative' as const, label: 'Needs Improvement (1-3★)' },
        ].map((f) => (
          <button
            key={f.key}
            type="button"
        ...
        <div className="space-y-3">
          {filtered.map((r, i) => (
            ...
            >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`w-3 h-3 ${s <= (r.rating ?? 0) ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30'}`}
                          />
                        ))}
                      </div>
              ...
              {r.comment && <p className="mt-3 text-sm text-muted-foreground bg-muted/20 rounded-lg p-3">{r.comment}</p>}
            </motion.div>
          ))}
        </div>
      )}
