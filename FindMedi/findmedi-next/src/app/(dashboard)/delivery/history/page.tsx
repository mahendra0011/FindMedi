...
import { Clock, CheckCircle, XCircle, MapPin, Search, IndianRupee, Calendar } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { useMyDeliveries } from '@/features/delivery';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{history.length} past deliveries</p>
      </div>

      <div className="flex gap-2 flex-wrap">
        {(['All', 'Delivered', 'Failed', 'Cancelled'] as const).map((f) => (
          <button
            key={f}
            type="button"
            onClick={() => setFilter(f)}
          ...
          >
            {f} ({f === 'All' ? history.length : history.filter((d) => d.status === f).length})
          </button>
        ))}
      </div>
          ...
          {filtered.map((d, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{d.dropAddress ?? 'Customer'}</p>
                  </div>
                </div>
              ...
              <div className="mt-3 flex items-start gap-2 text-xs text-muted-foreground">
                ...
                <span>
                  {d.pickupAddress ?? 'Pharmacy'} → {d.dropAddress ?? 'Customer'}
                </span>
              </div>

              {d.trackingHistory && d.trackingHistory.length > 0 && (
