/**
 * Manage Departments — ported from client/src/pages/admin/AdminDepartments.jsx (Phase 4).
 * Department directory with fees structure and add/edit/remove.
 */
'use client';

import { useState } from 'react';
import { motion } from 'motion/react';
import { Building2, Plus, Trash2, Save, Edit, IndianRupee } from 'lucide-react';
...
import type { DepartmentForm } from '@/features/departments/types';

const emptyForm: DepartmentForm = { name: '', description: '', head: '', active: true, fees_structure: 0 };
...
export default function DepartmentsPage() {
  ...
  const { data: departmentsData, isLoading: loading } = useDepartments();
  ...
  const handleSave = () => {
    if (editId) {
      updateMut.mutate({ id: editId, ...form }, { onSuccess: resetForm, onError: onError('Failed to update department') });
    } else {
      createMut.mutate(form, { onSuccess: resetForm, onError: onError('Failed to create department') });
    }
  };

  const handleEdit = (dept: { _id: string; name: string; description?: string; head?: string; active?: boolean; fees_structure?: number }) => {
    setForm({
      name: dept.name,
      description: dept.description ?? '',
      head: dept.head ?? '',
      active: dept.active ?? true,
      fees_structure: dept.fees_structure ?? 0,
    });
    setEditId(dept._id);
    setShowForm(true);
  };
  ...
  const handleDelete = (id: string) => {
    ...
    deleteMut.mutate(id, { onError: onError('Failed to delete department') });
  };
    ...
    <div className="space-y-6">
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-semibold ${dept.active ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}`}
                    >
              ...
              <div className="space-y-1.5 text-sm text-muted-foreground mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>Fees: ₹{dept.fees_structure}</span>
                </div>
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={resetForm}>
          ...
          >
            <h3 className="text-lg font-bold text-foreground mb-4">{editId ? 'Edit Department' : 'Add New Department'}</h3>
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Department Head</label>
                <Input value={form.head} onChange={(e) => setForm({ ...form, head: e.target.value })} placeholder="Dr. Name" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Fees Structure (₹)</label>
                <Input
                  type="number"
                  value={form.fees_structure}
                  onChange={(e) => setForm({ ...form, fees_structure: Number(e.target.value) })}
                  placeholder="500"
                />
              </div>
              <div className="flex items-center gap-2">
