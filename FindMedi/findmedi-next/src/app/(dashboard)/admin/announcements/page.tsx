/**
 * Announcements — ported from client/src/pages/admin/AdminAnnouncements.jsx
 */
'use client';
...
const priorityIcons: Record<string, typeof Info> = { urgent: AlertCircle, high: AlertTriangle, normal: Info, low: Info };
...
export default function AnnouncementsPage() {
  ...
  const { data: announcementsData, isLoading: loading } = useQuery({
    ...
    queryFn: async (): Promise<AnnouncementRow[]> => {
      ...
      return (Array.isArray(res) ? res : []) as unknown as AnnouncementRow[];
    },
    staleTime: 30_000,
  });

  const createMut = useMutation({
    mutationFn: (body: Record<string, unknown>) => announcementsApi.create(body),
    onSuccess: () => {
  ...
  const handleSend = () => {
    ...
    createMut.mutate({ title, message, priority, targetRoles });
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Title</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Staff Meeting Tomorrow" />
          </div>
          <div className="space-y-2">
            <Label>Message</Label>
            <Textarea rows={4} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type your announcement here..." />
          </div>
          ...
          <Button onClick={handleSend} disabled={createMut.isPending} className="gap-2 w-full">
            {createMut.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
            {createMut.isPending ? 'Sending...' : 'Send Announcement'}
          </Button>
        </CardContent>
      </Card>

      <div className="space-y-4">
        <h2 className="text-lg font-semibold text-foreground">Announcement History</h2>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground flex-wrap">
                      <span className="flex items-center gap-1">
                        <User className="w-3 h-3" /> {a.createdBy?.name || 'hospital_admin'}
                      </span>
                      <span className="flex items-center gap-1">
