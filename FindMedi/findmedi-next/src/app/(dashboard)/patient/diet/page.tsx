/**
 * Diet & Kitchen — ported from client/src/pages/DietKitchen.jsx (Phase 4).
 * Diet orders, meal delivery log, dietitian review, patient feedback.
 */
'use client';

import { useState } from 'react';
import { Search, Plus, Clock, X, UtensilsCrossed, CheckCircle, ThumbsUp, AlertTriangle, FileText, ChefHat, Send, Star } from 'lucide-react';
...
import type { DietOrder } from '@/features/diet/types';

const dietTypeColors: Record<string, string> = {
  Regular: 'bg-success/10 text-success',
  Diabetic: 'bg-info/10 text-info',
  'Low Sodium': 'bg-primary/10 text-primary',
  Liquid: 'bg-warning/10 text-warning',
  Soft: 'bg-orange-500/10 text-orange-600',
  'High Protein': 'bg-purple-500/10 text-purple-600',
  'Low Fat': 'bg-green-500/10 text-green-600',
  Renal: 'bg-red-500/10 text-red-600',
  NPO: 'bg-destructive/10 text-destructive',
  Other: 'bg-muted text-muted-foreground',
};

const mealTypes = ['Breakfast', 'Lunch', 'Evening Snack', 'Dinner'];

export default function DietKitchenPage() {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [showCreate, setShowCreate] = useState(false);
  const [showReview, setShowReview] = useState(false);
  const [reviewOrder, setReviewOrder] = useState<DietOrder | null>(null);
  const [reviewNotes, setReviewNotes] = useState('');
  const [reviewStatus, setReviewStatus] = useState('Approved');
  const [showFeedback, setShowFeedback] = useState<DietOrder | null>(null);
  ...
  const { data: ordersData, isLoading } = useDietOrders(search, statusFilter);
  const { data: statsData } = useDietStats();

  const createMut = useCreateDietOrder();
  const deliverMut = useDeliverMeal();
  const confirmMut = useConfirmMeal();
  const reviewMut = useReviewDiet();
  const feedbackMut = useAddDietFeedback();
  const notifyMut = useNotifyKitchen();
  const billingMut = useAddDietToBilling();

  const orders: DietOrder[] = ordersData?.orders ?? [];
  const stats = statsData ?? { active: 0, todayMeals: 0, pendingReview: 0, total: 0 };

  const renderFeedbackForm = (order: DietOrder) => (
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="flex gap-2">
            {['Good', 'Average', 'Bad'].map((t) => (
              <button
                key={t}
                onClick={() => setFeedbackData((f) => ({ ...f, taste: t }))}
        ...
        <div>
          ...
          <div className="flex gap-2">
            {['Adequate', 'Too Much', 'Too Little'].map((q) => (
              <button
                key={q}
                onClick={() => setFeedbackData((f) => ({ ...f, quantity: q }))}
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() =>
              feedbackMut.mutate(
                { id: order._id, mealIndex: 0, ...feedbackData },
          ...
          <Button size="sm" variant="outline" onClick={() => setShowFeedback(null)}>
            Cancel
          </Button>
        </div>
      </div>
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto py-6">
        <p className="text-muted-foreground">Loading diet data…</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-bold">Diet & Kitchen</h1>
        <p className="text-sm text-muted-foreground">
          {stats.active} active orders · {stats.todayMeals} meals today · {stats.pendingReview} pending review
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { l: 'Active Orders', v: stats.active, c: 'text-primary', ic: UtensilsCrossed },
          { l: "Today's Meals", v: stats.todayMeals, c: 'text-success', ic: CheckCircle },
          { l: 'Pending Review', v: stats.pendingReview, c: 'text-warning', ic: ChefHat },
          { l: 'Total Orders', v: stats.total, c: 'text-foreground', ic: FileText },
        ].map((s) => (
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <Button onClick={() => setShowCreate(true)}>
          <Plus className="w-4 h-4 mr-1" /> New Diet Order
        </Button>
      </div>

      <div className="space-y-4">
        {orders.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-10">No diet orders found.</p>
        ) : (
          orders.map((order) => {
            ...
            return (
              <div
                key={order._id}
                className={`bg-card rounded-xl border shadow-sm ${order.status === 'Pending Review' ? 'ring-1 ring-warning/30 border-warning/20' : ''}`}
              >
                <div className="p-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : order._id)}>
                  <div className="flex items-center gap-3">
                    <UtensilsCrossed className="w-5 h-5 text-primary" />
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${dietTypeColors[order.dietType ?? ''] ?? ''}`}>
                      {order.dietType}
                    </span>
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${order.status === 'Active' ? 'bg-success/10 text-success' : order.status === 'Pending Review' ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'}`}
                    >
                      {order.status}
                    </span>
                  ...
                  <div className="px-4 pb-4 border-t pt-3 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{order.doctorName || 'Not specified'}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">{order.allergies || 'None'}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Meals</span>
                        <p className="font-medium">{order.mealTimes?.join(', ') || 'N/A'}</p>
                      </div>
                    </div>

                    {order.reviewedByDietitian ? (
                      <div
                        className={`rounded-lg p-2 text-xs ${reviewStatus === 'Approved' ? 'bg-success/5 text-success' : 'bg-warning/5 text-warning'}`}
                      >
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="text-xs font-medium text-foreground">
                                  {m.mealType} · {m.date ? new Date(m.date).toLocaleDateString() : 'Today'}
                                </p>
                                <p className="text-[10px] text-muted-foreground">
                                  {m.items || 'No items'} · Delivered by {m.deliveredBy || 'N/A'}
                                </p>
                              </div>
                              <div className="flex items-center gap-2">
                                {m.patientFeedback ? (
                                  <div className="flex items-center gap-1">
                                    {m.patientFeedback === 'Good' ? (
                                  ...
                                  <Button
                                    ...
                                    variant="ghost"
                                    onClick={() => {
                                      setShowFeedback(order);
                                      setFeedbackData({ taste: 'Good', quantity: 'Adequate', comments: '' });
                                    }}
                                  >
                                ...
                                {!m.confirmedByNurse ? (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => confirmMut.mutate({ id: order._id, mealIndex: i, feedback: 'Good' })}
                                  >
                                    Confirm
                                  </Button>
                                ) : (
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (m && mealTypes.includes(m))
                            deliverMut.mutate({ id: order._id, mealType: m, items: prompt('Items:') || '' });
                        }}
                      >
                      ...
                      {!order.billingAdded && order.status === 'Completed' && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (amt) billingMut.mutate({ id: order._id, amount: parseInt(amt, 10) });
                          }}
                        >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input value={newOrder.ward} onChange={(e) => setNewOrder({ ...newOrder, ward: e.target.value })} />
                </div>
                <div>
                  <label className="text-sm font-medium mb-1 block">Bed Number</label>
                  <Input value={newOrder.bedNumber} onChange={(e) => setNewOrder({ ...newOrder, bedNumber: e.target.value })} />
                </div>
                <div>
                  ...
                  >
                    {['Regular', 'Diabetic', 'Low Sodium', 'Liquid', 'Soft', 'High Protein', 'Low Fat', 'Renal', 'NPO', 'Other'].map(
                      (d) => (
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Meal Times</label>
                <div className="flex gap-2 flex-wrap">
                  {mealTypes.map((m) => (
                    <label key={m} className="flex items-center gap-1 text-sm">
                      <input
                        type="checkbox"
                        checked={newOrder.mealTimes.includes(m)}
                        onChange={() =>
                          setNewOrder((o) => ({
                            ...o,
                            mealTimes: o.mealTimes.includes(m) ? o.mealTimes.filter((x) => x !== m) : [...o.mealTimes, m],
                          }))
                        }
                        className="w-4 h-4"
                      />
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. No salt, pureed food, tube feed..."
                />
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Allergies</label>
                <Input value={newOrder.allergies} onChange={(e) => setNewOrder({ ...newOrder, allergies: e.target.value })} />
              </div>
              <Button
                className="w-full"
                onClick={() => createMut.mutate({ ...newOrder }, { onSuccess: () => setShowCreate(false) })}
                disabled={createMut.isPending || !newOrder.patientName}
              >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowReview(false)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Dietitian Review</h3>
            ...
            <div className="space-y-4">
              <div className="text-sm">
                ...
                <p className="text-muted-foreground">
                  {reviewOrder.dietType} diet · {reviewOrder.mealTimes?.join(', ')}
                </p>
              ...
              <Button
                ...
                onClick={() => {
                  reviewMut.mutate(
                    { id: reviewOrder._id, status: reviewStatus, notes: reviewNotes },
