/**
 * Help & Support — ported from client/src/pages/patient/PatientSupport.jsx (Phase 4).
...
import { HelpCircle, MessageSquare, AlertCircle, TicketCheck, Clock, CheckCircle2 } from 'lucide-react';
...
const ticketStatusIcons: Record<string, LucideIcon> = {
  Open: AlertCircle,
  'In Progress': Clock,
...
export default function SupportPage() {
  ...
  const { data: ticketsData, isLoading: ticketsLoading } = useQuery({
    ...
    queryFn: async (): Promise<SupportTicket[]> => {
      const res = await supportTickets.getMy({});
      return (Array.isArray(res) ? res : []) as unknown as SupportTicket[];
    },
    staleTime: 30_000,
  });

  const createMut = useMutation({
    mutationFn: (body: { subject: string; message: string }) => supportTickets.create(body),
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t.ticketId} · {t.createdAt ? new Date(t.createdAt).toLocaleDateString() : ''}
                          </p>
                        </div>
                        <span
                          className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium flex items-center gap-1 ${ticketStatusColors[t.status ?? ''] ?? 'bg-muted text-muted-foreground'}`}
                        >
        ...
        >
          ...
          <div className="p-6 rounded-2xl bg-muted/30 border border-border/50">
            ...
            <p className="text-sm text-muted-foreground">
              For medical emergencies, please do not use this support form. Contact the nearest emergency service or use our
              Emergency Services feature immediately.
            </p>
          </div>
        </motion.div>
      </div>
