/**
 * My Refunds — ported from client/src/pages/patient/PatientRefunds.jsx (Phase 4).
 * Wired to the real refunds endpoint (the old client left this as a placeholder).
 */
'use client';
...
import { RotateCcw, Search, Calendar, IndianRupee, AlertCircle, Loader2, FileText } from 'lucide-react';
...
const fallbackStatus = { label: 'Pending', color: 'bg-muted text-muted-foreground', icon: FileText };
...
export default function RefundsPage() {
  ...
  const { data: refundsData, isLoading: loading } = useQuery({
    queryKey: ['patient-refunds'],
    queryFn: async (): Promise<RefundItem[]> => {
      const res = await payments.getRefunds({});
      return (Array.isArray(res) ? res : []) as unknown as RefundItem[];
    },
    staleTime: 30_000,
  });
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex items-start gap-3.5 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-foreground text-sm">#{r.refundId}</p>
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${(statusConfig[r.status ?? ''] ?? fallbackStatus).color}`}
                  >
                    {(statusConfig[r.status ?? ''] ?? fallbackStatus).label}
                  </span>
                </div>
