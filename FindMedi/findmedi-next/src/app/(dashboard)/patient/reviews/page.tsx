/**
 * My Reviews — ported from client/src/pages/patient/PatientReviews.jsx (Phase 4).
 * Patient's own reviews with rating filters and delete.
 */
'use client';
...
function getAvatarColor(name?: string) {
  if (!name) return avatarColors[0] ?? 'bg-sky-500';
  let hash = 0;
  for (let i = 0; i < name.length; i++) hash = name.charCodeAt(i) + ((hash << 5) - hash);
  return avatarColors[Math.abs(hash) % avatarColors.length] ?? 'bg-sky-500';
}
...
export default function MyReviewsPage() {
  ...
  const { data: reviewsData, isLoading: loading } = useQuery({
    queryKey: ['patient-reviews', user?.name ?? ''],
    queryFn: async (): Promise<PatientReview[]> => {
      const res = await reviewsApi.get({});
      const list = (Array.isArray(res) ? res : []) as unknown as PatientReview[];
      return list.filter((rv) => rv.patientName === user?.name);
    },
    staleTime: 30_000,
  });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      await qc.invalidateQueries({ queryKey: ['patient-reviews'] });
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredReviews.map((rv, i) => (
            ...
            >
              <button
                onClick={() => void handleDelete(rv._id)}
              ...
              <div className="flex items-start gap-3.5 mb-3.5">
                <div
                  className={`w-10 h-10 rounded-xl ${getAvatarColor(rv.doctorName)} flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-sm`}
                >
              ...
              {rv.comment && <p className="text-sm text-foreground/85 leading-relaxed mb-3.5 line-clamp-3">&ldquo;{rv.comment}&rdquo;</p>}
