/**
 * Write a Review — ported from client/src/pages/patient/PatientWriteReview.jsx (Phase 4).
 * Star rating + comment for a chosen doctor.
 */
'use client';
...
import { doctors, reviews as reviewsApi } from '@/lib/api';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => router.push('/patient/reviews')} className="p-2 rounded-xl hover:bg-muted transition-colors">
      ...
      >
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((s) => (
                ...
                >
                  <Star
                    className={`w-8 h-8 transition-colors ${s <= (hoverRating || rating) ? 'text-warning fill-warning' : 'text-muted'}`}
                  />
                </button>
              ))}
            </div>
        ...
        <div className="flex gap-3 mt-6">
          <Button variant="outline" className="flex-1" onClick={() => router.push('/patient/reviews')}>
            Cancel
          </Button>
          <Button className="flex-1 gap-2" onClick={() => void handleSubmit()} disabled={!selectedDoctor || !rating || submitting}>
