/**
 * My Schedule — ported from client/src/pages/doctor/DoctorSchedule.jsx (Phase 4).
 * Read-only weekly schedule, time slots, leaves, profile details.
 */
'use client';

import { Clock, Calendar, Settings, Info } from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';
import { useMyDoctor } from '@/features/doctor/hooks';
import type { WeeklySchedule } from '@/features/doctor/types';
      ...
      <div className="bg-info/10 border border-info/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            This is a view-only schedule. Please contact the hospital administration for any changes.
          </p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border/60 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-foreground">{doctor.experience}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Consultation Fees</p>
            <p className="font-medium text-foreground">₹{doctor.consultation_fees}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Location</p>
            <p className="font-medium text-foreground">{doctor.location || 'Not set'}</p>
          </div>
          <div>
