/**
 * Consultations — ported from client/src/pages/doctor/DoctorConsultations.jsx (Phase 4).
 * Doctor's own consultation records with type filters and PDF download.
 */
'use client';
...
import { useMedicalRecords, useCreateRecord, useDownloadRecordPdf } from '@/features/records/hooks';
...
const fallbackIcon = { icon: Activity, color: 'text-primary', bg: 'bg-primary/10' };

const typeFilters = ['All', 'Prescription', 'Lab Report', 'Diagnosis'];
...
export default function ConsultationsPage() {
  ...
  const downloadMut = useDownloadRecordPdf();

  const userName = user?.name?.toLowerCase() ?? '';
  const records = (recordsData ?? []).filter((r) => (r.doctor ?? '').toLowerCase().includes(userName));
  ...
  const handleDownloadPdf = (recordId: string) => {
    downloadMut.mutate(recordId, { onError: (e: Error) => toast.error(e.message) });
  };
    ...
    <div className="space-y-6">
      ...
      {filteredRecords.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredRecords.map((rec, i) => {
            const cfg = typeIcons[rec.type ?? ''] ?? fallbackIcon;
            const Icon = cfg.icon;
                ...
                <div onClick={() => setExpanded(isExpanded ? null : rec._id)} className="p-5 cursor-pointer hover:bg-muted/30 transition-all">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-semibold text-foreground">{rec.patient}</h3>
                    ...
                    <div className="text-right">
                      ...
                      {isExpanded ? (
                        <ChevronUp className="w-5 h-5 text-muted-foreground mt-2" />
                      ) : (
                        ...
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div>
                              ...
                              <span className="text-foreground ml-2">{rec.patient}</span>
                            </div>
                            {rec.data?.patient?.age && (
                              <div>
                                <span className="text-muted-foreground">Age:</span>{' '}
                                <span className="text-foreground ml-2">{rec.data.patient.age}</span>
                              </div>
                            )}
                            {rec.data?.patient?.gender && (
                              <div>
                                <span className="text-muted-foreground">Gender:</span>{' '}
                                <span className="text-foreground ml-2">{rec.data.patient.gender}</span>
                              </div>
                            )}
                            {rec.data?.patient?.phone && (
                              <div>
                                <span className="text-muted-foreground">Phone:</span>{' '}
                                <span className="text-foreground ml-2">{rec.data.patient.phone}</span>
                              </div>
                            )}
                          </div>
                        </div>
                          ...
                          <div className="md:col-span-2">
                            ...
                            <div className="bg-success/5 rounded-xl p-4">
                              <pre className="text-sm text-foreground whitespace-pre-wrap">
                                {rec.prescription || rec.data?.medications?.join('\n')}
                              </pre>
                            </div>
                          </div>
                        )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          ...
          >
            <h3 className="text-xl font-bold text-foreground mb-4">New Consultation Record</h3>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <Button className="flex-1 gap-2" onClick={handleSave} disabled={!patientName || !diagnosis || createMut.isPending}>
