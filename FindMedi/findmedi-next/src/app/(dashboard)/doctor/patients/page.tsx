/**
 * Patients Management — ported from client/src/pages/Patients.jsx (Phase 4).
 * Patient directory with status filters, stats, add/remove.
 */
'use client';
...
export default function PatientsPage() {
  ...
  const { data: patientsData, isLoading } = useDoctorPatients(search, statusFilter);
  const createMut = useCreateDoctorPatient();
  const deleteMut = useDeleteDoctorPatient();
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {statCards.map((s, i) => (
            ...
            >
              <div
                className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${s.gradient} flex items-center justify-center mb-3 shadow-sm group-hover:scale-110 transition-transform`}
              >
                <s.icon className={`w-5 h-5 ${s.color}`} />
              </div>
              <p className="text-3xl font-bold text-foreground tracking-tight">{statValues[i] ?? 0}</p>
                ...
                patients.map((p, idx) => (
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold shrink-0 shadow-sm ${
                            p.status === 'Critical'
                    ...
                    <td className="px-5 py-4">
                      <button
                        onClick={() => {
                          if (confirm('Remove patient?')) deleteMut.mutate(p._id);
                        }}
        ...
        >
          ...
          >
            ...
            >
              ...
              {createMut.error && <p className="text-sm text-destructive">{(createMut.error as Error).message}</p>}
              <div className="flex gap-3 pt-2">
                <Button type="button" variant="outline" className="flex-1 rounded-xl" onClick={() => setModal(false)}>
                  Cancel
                </Button>
