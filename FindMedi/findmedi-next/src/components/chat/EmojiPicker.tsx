 ...
 *  - GIF: Giphy public beta key search
 */
export default function EmojiPicker({
  ...
  const tabs: Array<{ key: 'emoji' | 'sticker' | 'gif'; icon: React.ComponentType<{ className?: string }>; label: string }> = [
  ...
  return (
    <div
      className={`w-[330px] max-w-[92vw] bg-popover border border-border rounded-xl shadow-2xl overflow-hidden chat-pop-enter ${className}`}
    >
      ...
      <div className="h-[240px] overflow-y-auto scrollbar-thin px-2 py-2">
        {tab === 'emoji' && (
          <>
                ...
                <div className="flex flex-wrap gap-0.5">
                  {recent.map((e) => (
                    <button
                      key={e}
                      onClick={() => pickEmoji(e)}
                      className="w-8 h-8 rounded-lg text-[18px] hover:bg-muted"
                    >
                      {e}
                    </button>
                  ))}
            ...
            {!groups.length && (
              <p className="text-center text-[12px] text-muted-foreground py-8">No emoji found</p>
            )}
          </>
        )}
        ...
        {tab === 'gif' && (
          <>
            {gifLoading && (
              <p className="text-center text-[12px] text-muted-foreground py-8">Loading GIFs…</p>
            )}
            {gifError && !gifLoading && (
              <p className="text-center text-[12px] text-muted-foreground py-8">{gifError}</p>
            )}
            <div className="grid grid-cols-2 gap-1.5">
              {gifs.map((g) => (
                <button
                  ...
                  className="rounded-lg overflow-hidden border border-border hover:border-primary/50"
                >
