...
interface MediaViewerProps {
  ...
  onSendToChat?: (item: MediaItem, caption: string) => void;
}

/**
 * Fullscreen media viewer — image zoom/pan, video player, gallery navigation,
 * download and quick actions.
 */
export default function MediaViewer({
  ...
  const next = () => {
    if (items.length) setCurrent((c) => (c + 1) % items.length);
  };
  const prev = () => {
    if (items.length) setCurrent((c) => (c - 1 + items.length) % items.length);
  };
      ...
      <div className="flex items-center justify-between px-3 sm:px-5 py-3 border-b border-white/10 text-white">
        <div className="min-w-0">
          <p className="text-sm font-medium truncate">
            {item.name || (isImage ? 'Photo' : isVideo ? 'Video' : 'File')}
          </p>
      ...
      >
        ...
        {items.length > 1 && (
          <>
            <button
              onClick={prev}
              className="absolute left-2 sm:left-5 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>
            <button
              onClick={next}
              className="absolute right-2 sm:right-5 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center"
            >
