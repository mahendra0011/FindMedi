...
/* ─── Animated Counter ────────────────────────────── */
function AnimatedCounter({ value, prefix = '', suffix = '', duration = 1200 }: { value: number; prefix?: string; suffix?: string; duration?: number }) {
...
/* ─── Heatmap Day Cell ────────────────────────────── */
function HeatmapCell({ intensity, day, amount }: { intensity: number; day: string; amount: number }) {
...
export default function EarningsAnalytics({ bills = [], payments = [], title = 'Earnings Analytics' }: EarningsAnalyticsProps) {
  const [range, setRange] = useState<'6m' | '30d'>('6m');
  const [showAmounts, setShowAmounts] = useState(true);
  const [activeChart, setActiveChart] = useState<'area' | 'bar' | 'composed'>('area');
  ...
  const stats = useMemo(() => {
    ...
    const now = new Date();
    const monthlyMap: Record<string, { earned: number; billed: number; count: number }> = {};
    bills.forEach((b) => {
      if (!b.date) return;
    ...
    const last30: Array<{ day: string; amount: number; intensity?: number }> = [];
    let maxDayEarned = 0;
    for (let i = 29; i >= 0; i--) {
      const d = new Date(now);
      d.setDate(now.getDate() - i);
    ...
    return {
      ...
      txCount, avgTicket, bestDay,
      todayEarned, weekEarned, monthEarned,
      recent, last30, peakHours, streak, thisMonth, lastMonth
    };
  }, [bills, payments]);

  const chartData = range === '6m' ? stats.monthlyTrend : stats.weekly;
  const fmt = (v: number) => showAmounts ? `₹${v.toLocaleString('en-IN')}` : '₹••••••';
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1 bg-muted/50 rounded-xl p-1 border border-border/30">
            {[
              { k: '6m' as const, l: '6 Months', icon: CalendarRange },
              { k: '30d' as const, l: '7 Days', icon: CalendarDays },
            ].map((opt) => (
              <button
                key={opt.k}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { icon: Trophy,     label: 'Best Day',     value: stats.bestDay && stats.bestDay.earned > 0 ? `${stats.bestDay.label} · ${fmt(stats.bestDay.earned)}` : '—', color: 'text-amber-500', bg: 'bg-amber-500/10' },
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-0.5 bg-muted/50 rounded-lg p-0.5 border border-border/30">
              {[
                { k: 'area' as const, icon: LineChartIcon },
                { k: 'bar' as const, icon: BarChart3 },
                { k: 'composed' as const, icon: Layers },
              ].map(opt => (
                <button
                  key={opt.k}
          ...
          <TooltipProvider>
            <div className="grid grid-cols-10 gap-1.5">
              {stats.last30.map((d, i) => (
                <HeatmapCell key={i} intensity={d.intensity || 0} day={d.day} amount={d.amount} />
              ))}
            </div>
          </TooltipProvider>
