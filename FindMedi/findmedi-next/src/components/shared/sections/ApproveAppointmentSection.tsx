 ...
 *
 * Ported from client/src/components/ApproveAppointmentSection.jsx.
 * Migration changes:
 *   - useNavigate → useRouter (Next.js App Router)
 *   - formatDisplayDate from @/lib/utils
 *   - TypeScript prop types throughout
 *   - api.* methods from @/lib/api/endpoints (typed)
 */
'use client';
...
import { getISTDateString, formatDisplayDate } from '@/lib/utils';
import { resolveFileUrl, isValidFileUrl } from '@/lib/utils';
import {
...
interface DisplayAppointment extends Omit<Appointment, 'doctor' | 'doctorId'> {
  patient?: string;
  doctorName?: string;
...
interface ApproveAppointmentSectionProps {
  ...
  onConfirm?: (apt: DisplayAppointment) => void;
  onReject?: (apt: DisplayAppointment, reason: string) => void;
}

export default function ApproveAppointmentSection({ appointments, onConfirm, onReject }: ApproveAppointmentSectionProps) {
  ...
  const [rejectTarget, setRejectTarget] = useState<DisplayAppointment | null>(null);
  ...
  const getFirstDay = (date: Date): number => new Date(date.getFullYear(), date.getMonth(), 1).getDay();
  ...
  const subSlots = useMemo(() => {
    ...
    const present = [...new Set(sorted.map(a => subSlotFor(a.time)).filter((s): s is string => s !== null))];
    if (present.length > 0) return present;
    const fallbackHour = hourBoxFor(sorted[0]?.time) || hourSlots[0];
  ...
  const pendingCount = (arr: DisplayAppointment[]): number =>
    (arr || []).filter(x => (x.status || '').toLowerCase() === 'pending').length;
  const rejectedCount = (arr: DisplayAppointment[]): number =>
    (arr || []).filter(x => (x.status || '').toLowerCase() === 'cancelled').length;
  ...
  const recent = useMemo(() => {
    return [...appointments]
      ...
      .sort((a, b) =>
        ...
        ((parseTime(b.time).hour ?? 0) - (parseTime(a.time).hour ?? 0))
      )
      .slice(0, 30);
  }, [appointments]);
                        ...
                        {isRejected ? (
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button
                              ...
                              onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); setRejectTarget(apt); }}
...
interface ApproveCardProps {
  ...
  onConfirm?: (apt: DisplayAppointment) => void;
  onRejectClick?: (apt: DisplayAppointment) => void;
}

function ApproveCard({ apt, subSlotFor, onViewFile, onConfirm, onRejectClick }: ApproveCardProps) {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border/60 p-5 shadow-sm flex-1 flex flex-col mb-3">
      ...
      {showIntake && (
        <div className="mt-3 bg-muted/20 rounded-xl p-4 border border-border/40">
          <h4 className="font-heading text-sm font-bold text-foreground mb-3 flex items-center gap-1.5">
