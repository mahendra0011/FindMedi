...
import {
  CalendarClock, Clock, Video, Phone, MessageSquare,
  Search, Eye, Filter, Droplet, CalendarDays
} from 'lucide-react';
 ...
 */
function getRelativeDays(targetDate: string, todayStr: string): string {
  if (!targetDate) return '';
  if (targetDate === todayStr) return 'Today';

  const parts1 = todayStr.split('-').map(Number);
  const y1 = parts1[0] ?? 2026;
  const m1 = parts1[1] ?? 1;
  const d1 = parts1[2] ?? 1;
  const parts2 = targetDate.split('-').map(Number);
...
export default function UpcomingAppointmentsSection({
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState<'all' | 'tomorrow' | 'week'>('all');
                        ...
                        <div>
                          ...
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {apt.tokenNumber && (
                                <span className="inline-block mt-1 px-2 py-0.5 rounded-md bg-muted text-muted-foreground text-[10px] font-semibold">
