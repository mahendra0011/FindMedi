 ...
 *
 * Ported from client/src/components/PatientHistoryModal.jsx.
 * Migration changes:
 *   - framer-motion → motion/react
 *   - api import → request<T>() from @/lib/api/client
...
import type { AppointmentPatientRef } from '@/types/models/appointment';
...
export interface PatientHistoryRecord {
  ...
  symptoms?: string;
  tests?: { name?: string; testName?: string; result?: string; value?: string; unit?: string }[];
  vitals?: {
...
export const PATIENT_HISTORY_TYPE_LABELS: Record<string, string> = {
...
export const PATIENT_HISTORY_TYPE_COLORS: Record<string, string> = {
...
function HistoryRecordCard({ record }: { record: PatientHistoryRecord }) {
  ...
  const meds = record.data?.medications || record.medications;
  const doctorName = record.doctor || record.doctorId?.name || '';
  const type = record.type || 'diagnosis';
  const chiefComplaints = record.data?.chiefComplaints || record.chiefComplaints || '';
  const tests = record.data?.tests || record.tests || [];
  const symptoms = record.data?.symptoms || record.symptoms || '';
    ...
    <div className="bg-muted/20 rounded-xl border border-border/50 p-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium text-foreground">{date ? formatDate(date) : date || '—'}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${PATIENT_HISTORY_TYPE_COLORS[type] || PATIENT_HISTORY_TYPE_COLORS.diagnosis}`}>
            {PATIENT_HISTORY_TYPE_LABELS[type] || type}
          </span>
          {doctorName && (
...
export default function PatientHistoryModal({ patient, patientName, onClose }: PatientHistoryModalProps) {
