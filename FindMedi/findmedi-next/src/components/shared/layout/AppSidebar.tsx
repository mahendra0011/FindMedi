/**
 * AppSidebar — role-based navigation sidebar for the dashboard.
 *
 * Ported from client/src/components/AppSidebar.jsx.
 * Migration changes:
 *   - react-router-dom Link/useNavigate/useLocation → next/link + next/navigation useRouter/usePathname
 *   - useAuth from @/context/AuthContext → @/hooks/useAuth
 *   - t() translation → simple key lookup (i18n not yet ported)
 *   - NotificationBell import path → @/components/shared/notifications/NotificationBell
 *   - TypeScript types for nav config and props
 */
'use client';
...
const NAV_LABELS: Record<string, string> = {
  ...
  'nav.chatWithAI': 'AI Assistant',
  'nav.superAdminPanel': 'Super Admin',
  'nav.saOverview': 'Overview',
  'nav.saPending': 'Pending Approvals',
  'nav.saFacilities': 'Facilities',
  'nav.saStats': 'Statistics',
  'nav.saUsers': 'Users',
  'nav.saModeration': 'Moderation',
  'nav.saDisputes': 'Disputes',
  'nav.saRevenue': 'Revenue',
  'nav.verifyTransaction': 'Verify Transactions',
  'nav.saLicenses': 'Licenses',
  'nav.saCategories': 'Categories',
  'nav.saCatalog': 'Catalog',
  'nav.saAudit': 'Audit Logs',
  'nav.saBroadcast': 'Broadcast',
  'nav.saTickets': 'Support Tickets',
  'nav.saSettings': 'Settings',
  'nav.saTeam': 'Team',
  'nav.saPromotions': 'Promotions',
  'nav.saExport': 'Export',
  'nav.saCities': 'Cities',
  ...
  'nav.managePatients': 'My Patients',
  'nav.appointments': 'Appointments',
  'nav.medicalRecords': 'Medical Records',
  'nav.billing': 'Billing',
  'nav.prescriptionVerification': 'Prescription Verification',
  'nav.departments': 'Departments',
  'nav.reviews': 'Reviews',
  'nav.analytics': 'Analytics',
  'nav.emergency': 'Emergency',
  'nav.bedManagement': 'Bed Management',
  'nav.testCatalog': 'Test Catalog',
  'nav.ipd': 'IPD',
  'nav.ot': 'OT',
  'nav.staff': 'Staff',
  ...
  'nav.dietKitchen': 'Diet Kitchen',
  'nav.housekeeping': 'Housekeeping',
  'nav.triage': 'Triage',
  'nav.opdToken': 'OPD Token',
  ...
  'nav.deliveryZone': 'Delivery Zone',
  'nav.myDocuments': 'My Documents',
  'nav.labBookings': 'Lab Bookings',
  'nav.prescriptionQueue': 'Prescription Queue',
  'nav.sampleCollection': 'Sample Collection',
  'nav.labTests': 'Lab Tests',
  'nav.labEquipment': 'Lab Equipment',
  'nav.labPackages': 'Lab Packages',
  'nav.offers': 'Offers',
  'nav.returns': 'Returns',
  'nav.deliveries': 'Deliveries',
  'nav.pharmacySettings': 'Pharmacy Settings',
  'nav.home': 'Home',
  'common.logout': 'Logout',
  'role.superadmin': 'Super Admin',
  'role.hospital_admin': 'Hospital Admin',
  'role.doctor': 'Doctor',
  'role.clinic_doctor': 'Clinic Doctor',
  'role.patient': 'Patient',
  'role.lab_owner': 'Lab Owner',
  'role.pharmacy_owner': 'Pharmacy Owner',
  'role.delivery_boy': 'Delivery Boy',
};

const navConfig: NavConfig = {
  superadmin: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/superadmin' },
    ...
    { icon: FileCheck, labelKey: 'nav.verifyTransaction', path: '/admin/verify-transaction' },
    { icon: FileText, labelKey: 'nav.saLicenses', path: '/superadmin/licenses' },
    ...
    { icon: History, labelKey: 'nav.auditLogs', path: '/superadmin/audit-logs' },
  ],
  hospital_admin: [
    ...
    { icon: UserRound, labelKey: 'nav.managePatients', path: '/admin/patients' },
    { icon: CalendarDays, labelKey: 'nav.appointments', path: '/appointments' },
    { icon: FileText, labelKey: 'nav.medicalRecords', path: '/hospital/records' },
    ...
    { icon: Hospital, labelKey: 'nav.ipd', path: '/hospital/ipd' },
    { icon: Activity, labelKey: 'nav.ot', path: '/hospital/ot' },
    { icon: Users, labelKey: 'nav.staff', path: '/hospital/staff' },
    { icon: Heart, labelKey: 'nav.bloodBank', path: '/hospital/bloodbank' },
    { icon: Activity, labelKey: 'nav.physiotherapy', path: '/hospital/physio' },
    { icon: Brain, labelKey: 'nav.mentalHealth', path: '/patient/mentalhealth' },
    { icon: Baby, labelKey: 'nav.dietKitchen', path: '/patient/diet' },
    { icon: ClipboardList, labelKey: 'nav.housekeeping', path: '/hospital/housekeeping' },
    { icon: Activity, labelKey: 'nav.triage', path: '/hospital/triage' },
    { icon: Clock, labelKey: 'nav.opdToken', path: '/hospital/opd-token' },
    { icon: Activity, labelKey: 'nav.radiology', path: '/hospital/radiology' },
    { icon: Ambulance, labelKey: 'nav.nursing', path: '/hospital/nursing' },
    { icon: Pill, labelKey: 'nav.pharmacy', path: '/pharmacy' },
    { icon: FileText, labelKey: 'nav.inventory', path: '/hospital/inventory' },
    { icon: Download, labelKey: 'nav.reports', path: '/reports' },
    { icon: Syringe, labelKey: 'nav.lab', path: '/labcenter/lab' },
    { icon: Stethoscope, labelKey: 'nav.doctorConsultation', path: '/doctor/consultations' },
    { icon: FileUp, labelKey: 'nav.importExport', path: '/admin/import-export' },
  ...
  clinic_doctor: [
    ...
    { icon: Globe, labelKey: 'nav.platformSettings', path: '/patient/settings' },
    { icon: History, labelKey: 'nav.auditLogs', path: '/superadmin/audit-logs' },
  ],
  doctor: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/doctor' },
    { icon: Bot, labelKey: 'nav.chatWithAI', path: '/ai-chat' },
    ...
    { icon: TestTube, labelKey: 'nav.lab', path: '/labcenter/lab' },
  ...
  patient: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/patient' },
    { icon: Bot, labelKey: 'nav.chatWithAI', path: '/ai-chat' },
  ...
  lab_owner: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/lab-business' },
    { icon: Bot, labelKey: 'nav.chatWithAI', path: '/ai-chat' },
  ...
  pharmacy_owner: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/pharmacy-business' },
    { icon: Bot, labelKey: 'nav.chatWithAI', path: '/ai-chat' },
  ...
  delivery_boy: [
    { icon: LayoutDashboard, labelKey: 'nav.dashboard', path: '/delivery' },
    { icon: Bot, labelKey: 'nav.chatWithAI', path: '/ai-chat' },
...
const ROLE_BADGE_COLORS: Record<string, string> = {
  ...
  doctor: 'bg-info/20 text-info',
  patient: 'bg-success/20 text-success',
  clinic_doctor: 'bg-warning/20 text-warning',
  lab_owner: 'bg-purple-500/20 text-purple-600',
  pharmacy_owner: 'bg-rose-500/20 text-rose-600',
  delivery_boy: 'bg-blue-500/20 text-blue-600',
};
...
function SidebarContent({ collapsed, onToggleCollapse, onNavClick }: SidebarContentProps) {
  ...
  const handleGoHome = () => { router.push('/'); onNavClick?.(); };
  const handleLogout = () => { logout(); router.push('/login'); onNavClick?.(); };

  const navItems: NavItem[] = (navConfig[user?.role as UserRole] ?? navConfig.patient) as NavItem[];
    ...
    <div className={`flex flex-col h-full bg-sidebar text-sidebar-foreground ${collapsed ? 'w-[72px]' : 'w-64'}`}>
        ...
        <div className="mx-3 mt-3 p-3 bg-sidebar-accent/60 rounded-xl border border-sidebar-border/50">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full capitalize ${ROLE_BADGE_COLORS[user.role as string] || 'bg-muted text-muted-foreground'}`}>
                {NAV_LABELS[`role.${user.role}`] || user.role}
              </span>
            </div>
            <NotificationBell />
          </div>
      ...
      <nav className="sidebar-nav min-h-0 flex-1 py-3 px-2 space-y-0.5 overflow-y-auto overscroll-contain">
        {navItems.map(({ icon: Icon, labelKey, path }) => {
          ...
          const label = NAV_LABELS[labelKey] || labelKey;
          return (
            <Link key={path} href={path} onClick={onNavClick}
      ...
      <div className={`px-2 pb-4 border-t border-sidebar-border pt-3 space-y-1 ${collapsed ? 'flex flex-col items-center' : ''}`}>
        <button onClick={handleGoHome}
          title={collapsed ? NAV_LABELS['nav.home'] : undefined}
          ...
          {!collapsed && <span className="text-sm">{NAV_LABELS['nav.home']}</span>}
        </button>
        <button onClick={handleLogout}
          title={collapsed ? NAV_LABELS['common.logout'] : undefined}
          ...
          {!collapsed && <span className="text-sm">{NAV_LABELS['common.logout']}</span>}
        </button>
      </div>
