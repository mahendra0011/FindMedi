...
import IntakeFormStep, { type IntakeFormData } from './IntakeFormStep';
...
const emptyForm: PatientFormState = { name: '', age: '', gender: 'Male', phone: '', email: '', bloodGroup: '', address: '' };

const initialIntakeForm: IntakeFormData = {
...
export default function WalkInPatientForm({ timeSlots, onPatientCreated }: WalkInPatientFormProps) {
  const { user } = useAuth();
  ...
  const [, setFullSlots] = useState<string[]>([]);
  ...
  const [intakeForm, setIntakeForm] = useState<IntakeFormData>(initialIntakeForm);
  const [creating, setCreating] = useState(false);

  const set = (k: keyof PatientFormState, v: string) => setForm(f => ({ ...f, [k]: v }));

  const doctorId = (user as unknown as { doctorProfileId?: string })?.doctorProfileId || null;
  ...
  useEffect(() => {
    ...
    api.getDoctor(doctorId)
      .then(res => setDoctorProfile((res as unknown as { data?: DoctorProfileData })?.data || (res as unknown as DoctorProfileData)))
      .catch(() => setDoctorProfile(null));
  }, [doctorId]);

  const consultationFee = Number(doctorProfile?.consultation_fees) || Number((user as unknown as { consultationFee?: number; fees?: number })?.consultationFee) || Number((user as unknown as { fees?: number })?.fees) || 0;
  const slotDuration = Number(doctorProfile?.slotDuration) || (user as unknown as { slotDuration?: number })?.slotDuration || 15;
    ...
    api.getBookedSlots({ doctorId, date })
      .then(res => {
        ...
        if (data && typeof data === 'object' && !Array.isArray(data) && data.counts) {
          ...
          setFullSlots(data.fullSlots || Object.keys(data.counts || {}));
          setDateDisabledSlots(data.dateDisabled || []);
          setPendingDisabledSlots(data.pendingDisabledSlots || []);
        } else {
          const arr: string[] = Array.isArray(res) ? (res as string[]) : [];
          setFullSlots(arr);
  ...
  const isSlotFull = (time: string) => (slotCounts[time] || 0) >= 1;

  // Real-time hours: doctor schedule (time_slots), fallback = prop
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await api.walkInAppointment({
        patient: form.name,
        doctor: user?.name || '',
        department: (user as unknown as { department?: string })?.department || 'General',
        date,
        time: bookingTime,
        type: 'Consultation',
        notes,
      }) as unknown as { appointment?: { _id: string }; _id?: string; duplicate?: boolean };
      ...
      console.error(e);
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to book walk-in';
      toast.error(msg);
    } finally {
      setCreating(false);
    }
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-primary">{(user as unknown as { specialization?: string })?.specialization || 'Doctor'}</p>
            ...
            <div className="space-y-1.5">
              ...
              <select value={selectedHour || ''} onChange={e => { setSelectedHour(e.target.value); setBookingTime(''); }} className="w-full h-9 px-3 rounded-xl border border-border bg-background text-sm">
                ...
                {hourGroups.map(([hourLabel, hSlots]) => {
                  const allUnavailable = hSlots.every(s => isSlotFull(s) || isSlotDisabled(s) || isSlotPendingUpdate(s));
                  return (
                        ...
                        <button
                          ...
                          className={`flex flex-col items-center gap-0.5 px-1.5 py-2 rounded-lg border text-center transition-all ${
                            selected
                              ...
                              : unavailable
                              ? 'border-red-300 bg-red-50 text-red-600 dark:bg-red-950/20 dark:border-red-800/40 cursor-not-allowed'
              ...
              {bookingTime && (
                <div className="flex items-center gap-2 text-xs mt-1">
                  <div className={`px-2 py-1 rounded-md ${isSlotFull(bookingTime) ? 'bg-red-50 text-red-600 dark:bg-red-950/20' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20'}`}>
