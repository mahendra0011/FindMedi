/**
 * Mental Health — ported from client/src/pages/MentalHealth.jsx (Phase 4).
 * Cases, psychiatric assessment, MSE, treatment plans, sessions, confidentiality.
 */
'use client';

import { useState } from 'react';
import { Search, Plus, Clock, X, Brain, AlertTriangle, FileText, Lock, Shield, Activity, BookOpen } from 'lucide-react';
...
} from '@/features/mentalhealth/hooks';
import type { MHCase } from '@/features/mentalhealth/types';
...
const icd10Codes = [
  ...
  'Z03.2 - Observation for suspected mental disorder',
];
...
export function MentalHealthHub() {
  ...
  const [showAssessment, setShowAssessment] = useState<MHCase | null>(null);
  ...
  const [showMse, setShowMse] = useState<MHCase | null>(null);
  const [mseData, setMseData] = useState<Record<string, string>>({});
  const [showPlan, setShowPlan] = useState<MHCase | null>(null);
  ...
  const [showSession, setShowSession] = useState<MHCase | null>(null);
  const [sessionData, setSessionData] = useState({ therapyType: 'CBT', notes: '', patientResponse: 'Good', nextSession: '' });
  const [showConfidentiality, setShowConfidentiality] = useState<MHCase | null>(null);
  const [confData, setConfData] = useState({
    consentGiven: false,
    shareWithFamily: false,
    shareWithDoctor: true,
    accessLevel: 'Care Team Only',
    notes: '',
  });
  ...
  const { data: casesData, isLoading } = useMHCases(search, statusFilter);
  const { data: statsData } = useMHStats();

  const createMut = useCreateMHCase();
  const assessMut = useAddAssessment();
  const mseMut = useAddMse();
  const planMut = useCreateMHPlan();
  const sessionMut = useAddMHSession();
  const confMut = useUpdateConfidentiality();

  const cases: MHCase[] = casesData?.cases ?? [];
  const stats = statsData ?? { active: 0, critical: 0, followUp: 0, total: 0 };

  if (isLoading) {
    return (
      <div className="container mx-auto py-6">
        <p className="text-muted-foreground">Loading mental health data…</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          {stats.active} active · {stats.critical} critical · {stats.followUp} follow-up
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { l: 'Active Cases', v: stats.active, c: 'text-primary', ic: Brain },
          { l: 'Critical', v: stats.critical, c: 'text-destructive', ic: AlertTriangle },
          { l: 'Follow-up', v: stats.followUp, c: 'text-info', ic: Clock },
          { l: 'Total', v: stats.total, c: 'text-foreground', ic: FileText },
        ].map((s) => (
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        >
          ...
          {['Active', 'Under Treatment', 'Follow-up', 'Discharged', 'Referred'].map((s) => (
      ...
      <div className="space-y-4">
        {cases.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-10">No mental health cases found.</p>
        ) : (
          cases.map((c) => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {c.diagnosis || 'No diagnosis'} · Source: {c.source}
                      </p>
                    </div>
                    {(c.sessions?.length ?? 0) > 0 && (
                      <span className="text-xs text-muted-foreground">{c.sessions?.length} sessions</span>
                    )}
                  ...
                  <div className="px-4 pb-4 border-t pt-3 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{c.riskLevel}</p>
                      </div>
                      {c.assessment?.icdCode && (
                        <div className="col-span-2">
                          <span className="text-muted-foreground">ICD-10</span>
                          <p className="font-medium">{c.assessment.icdCode}</p>
                        </div>
                      )}
                      {c.notes && (
                        <div className="col-span-2">
                    ...
                    {c.assessment && (
                      <div className="bg-muted/20 rounded-lg p-3">
                        ...
                        <div className="text-xs space-y-1">
                          <p>
                            <span className="text-muted-foreground">Complaint:</span> {c.assessment.presentingComplaint}
                          </p>
                          {c.assessment.history && (
                            <p>
                              <span className="text-muted-foreground">History:</span> {c.assessment.history}
                            </p>
                          )}
                          {c.assessment.icdCode && (
                            <p>
                              <span className="text-muted-foreground">ICD-10:</span> {c.assessment.icdCode}
                            </p>
                          )}
                          <p>
                            <span className="text-muted-foreground">Diagnosis:</span> {c.assessment.diagnosis}
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {c.treatmentPlan && (
                      <div className="bg-primary/5 rounded-lg p-3">
                        ...
                        {c.treatmentPlan.medications && (
                          <p className="text-xs text-muted-foreground mt-1">Medications: {c.treatmentPlan.medications}</p>
                        )}
                      </div>
                    )}
                        ...
                        {c.sessions?.slice(-3).map((s, i) => (
                          <div key={i} className="bg-muted/30 rounded-lg p-2 text-xs">
                            <div className="flex justify-between">
                              ...
                              <span
                                className={`${s.patientResponse === 'Good' ? 'text-success' : s.patientResponse === 'Mixed' ? 'text-warning' : 'text-destructive'}`}
                              >
                                {s.patientResponse}
                              </span>
                            </div>
                            ...
                            {s.nextSession && (
                              <p className="text-muted-foreground mt-1">Next: {new Date(s.nextSession).toLocaleDateString()}</p>
                            )}
                          </div>
                        ))}
                      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAssessment(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Psychiatric Assessment</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Presenting Complaint *</label>
                <textarea
                  value={assessmentData.presentingComplaint}
                  onChange={(e) => setAssessmentData((d) => ({ ...d, presentingComplaint: e.target.value }))}
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">History (Personal, Family, Social)</label>
                <textarea
                  value={assessmentData.history}
                  onChange={(e) => setAssessmentData((d) => ({ ...d, history: e.target.value }))}
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">ICD-10 Code</label>
                <select
                  value={assessmentData.icdCode}
                  onChange={(e) => setAssessmentData((d) => ({ ...d, icdCode: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setAssessmentData((d) => ({ ...d, diagnosis: e.target.value }))}
                  placeholder="Clinical diagnosis..."
                />
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Risk Assessment</label>
                <div className="flex gap-2">
                  {['Low', 'Medium', 'High'].map((r) => (
                    <button
                      key={r}
                      onClick={() => setAssessmentData((d) => ({ ...d, riskLevel: r }))}
      ...
      {showMse && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowMse(null)}>
          <div
            className="bg-card rounded-2xl border shadow-xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Mental Status Examination (MSE)</h3>
              <button onClick={() => setShowMse(null)}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {mseCategories.map((cat) => (
                <div key={cat.key}>
                  ...
                  <select
                    value={mseData[cat.key] || ''}
                    onChange={(e) => setMseData((d) => ({ ...d, [cat.key]: e.target.value }))}
                    className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                  >
            ...
            <Button
              ...
              onClick={() => mseMut.mutate({ id: showMse._id, ...mseData }, { onSuccess: () => setShowMse(null) })}
            >
              Save MSE
            </Button>
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={planData.treatmentType}
                    onChange={(e) => setPlanData((d) => ({ ...d, treatmentType: e.target.value }))}
                    className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                  >
                ...
                <div>
                  ...
                  <Input value={planData.duration} onChange={(e) => setPlanData((d) => ({ ...d, duration: e.target.value }))} placeholder="e.g. 12 weeks" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setPlanData((d) => ({ ...d, medications: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm resize-none h-16"
      ...
      {showSession && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowSession(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Session Note</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Therapy Type</label>
                <select
                  value={sessionData.therapyType}
                  onChange={(e) => setSessionData((d) => ({ ...d, therapyType: e.target.value }))}
                  className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                >
                  <option>CBT</option>
                  <option>DBT</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setSessionData((d) => ({ ...d, notes: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm resize-none h-20"
                  placeholder="Content of session, observations..."
                />
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Patient Response</label>
                <div className="flex gap-2">
                  {['Good', 'Mixed', 'Poor'].map((r) => (
                    <button
                      key={r}
                      onClick={() => setSessionData((d) => ({ ...d, patientResponse: r }))}
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Next Session</label>
                <Input type="date" value={sessionData.nextSession} onChange={(e) => setSessionData((d) => ({ ...d, nextSession: e.target.value }))} />
              </div>
              <Button
                className="w-full"
                onClick={() =>
      ...
      {showConfidentiality && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowConfidentiality(null)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Confidentiality Settings</h3>
              <button onClick={() => setShowConfidentiality(null)}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={confData.consentGiven}
                  onChange={(e) => setConfData((d) => ({ ...d, consentGiven: e.target.checked }))}
                  className="w-4 h-4"
                />
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={confData.shareWithFamily}
                  onChange={(e) => setConfData((d) => ({ ...d, shareWithFamily: e.target.checked }))}
                  className="w-4 h-4"
                />
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={confData.shareWithDoctor}
                  onChange={(e) => setConfData((d) => ({ ...d, shareWithDoctor: e.target.checked }))}
                  className="w-4 h-4"
                />
              ...
              <div>
                ...
                <select
                  value={confData.accessLevel}
                  onChange={(e) => setConfData((d) => ({ ...d, accessLevel: e.target.value }))}
                  className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                >
                  <option>Care Team Only</option>
                  <option>Psychiatry Dept Only</option>
              ...
              <div>
                ...
                <textarea
                  value={confData.notes}
                  onChange={(e) => setConfData((d) => ({ ...d, notes: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm resize-none h-16"
                />
              </div>
              <Button
                className="w-full"
                onClick={() =>
                  confMut.mutate({ id: showConfidentiality._id, ...confData }, { onSuccess: () => setShowConfidentiality(null) })
                }
              >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-card rounded-2xl border shadow-xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">New Mental Health Case</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input value={newCase.patientName} onChange={(e) => setNewCase({ ...newCase, patientName: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-10 px-3 rounded-lg border border-input bg-background text-sm"
                  >
                    {['Doctor', 'Self', 'Family', 'Emergency', 'Counsellor'].map((s) => (
                      <option key={s} value={s}>
                        {s}
                      </option>
                    ))}
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Initial Diagnosis</label>
                <Input value={newCase.diagnosis} onChange={(e) => setNewCase({ ...newCase, diagnosis: e.target.value })} />
              </div>
              <div>
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={newCase.isConfidential}
                  onChange={(e) => setNewCase({ ...newCase, isConfidential: e.target.checked })}
                  className="w-4 h-4"
                />
              ...
              <Button
                className="w-full"
                onClick={() => createMut.mutate({ ...newCase }, { onSuccess: () => setShowCreate(false) })}
                disabled={createMut.isPending || !newCase.patientName}
              >
