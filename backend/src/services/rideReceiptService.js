...
export async function generateRideReceiptPdf(ride, user, rider, vehicle) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(8).fillColor(C.muted).text(`Authenticity Hash: ${signTxnRef(txnRef)}  (verify: /api/transactions/verify/${txnRef})`, 55, y + 58);
...
export function generateEscPosReceiptBytes(ride, user, rider, vehicle) {
  ...
  appendStr(`Txn Ref          : ${ride.payment?.transactionRef || ride.bookingNumber || ride._id}\n`);
  appendStr(`Auth Hash        : ${signTxnRef(ride.payment?.transactionRef || ride.bookingNumber || String(ride._id))}\n`);
  appendStr('--------------------------------\n');
