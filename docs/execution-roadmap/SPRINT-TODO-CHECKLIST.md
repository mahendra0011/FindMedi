...
- [ ] Refactor `backend/src/lib/h3Cache.js` to support multi-resolution binning (Res 6 for SOS, Res 7 for Lawyer/Doctor/Assistant, Res 8 for Rider).
- [ ] Implement Redis Key schema: `geo:h3:<resolution>:<cell>:<vertical>`.
- [ ] Add TTL auto-expiry (120s) to active provider keys to purge stale GPS devices.
...
- [ ] Create `backend/src/config/valhalla.js` HTTP/gRPC client pointing to Valhalla service (`http://valhalla-router:8002`).
- [ ] Implement `get1toNMatrix(pickupCoord, candidateCoords[])` returning road durations sorted ascending.
- [ ] Implement Haversine straight-line distance fallback in `backend/src/lib/geoUtils.js` if Valhalla times out (> 1500ms).
...
- [ ] Install / configure `ioredis` Redlock helper in `backend/src/lib/redlock.js`.
- [ ] Implement atomic provider lock: `SET lock:provider:<providerId> <bookingId> NX PX 15000`.
- [ ] Implement cascading dispatch queue in `backend/src/services/instantDispatchService.js`:
...
- [ ] Verify `frontend/src/components/emergency/ProviderIncomingCall.tsx` renders full-screen viewport (`fixed inset-0 z-[99999]`).
...
- [ ] Add Screen Wake Lock API (`navigator.wakeLock.request('screen')`) on modal mount.
- [ ] Add Hardware Haptic Vibration (`navigator.vibrate([500, 250, 500, 250])`).
- [ ] Add animated circular SVG countdown timer (12s to 60s).

### 1.5 Demo Payment Sandbox System
- [ ] Verify `backend/src/models/DemoPayment.js` and `backend/src/routes/demoPayment.js`.
- [ ] Implement 1-click Demo Success / Failure endpoints:
...
- [ ] Create frontend `<DemoPaymentCheckoutModal />` with instant virtual wallet balance (₹10,000 sandbox credit).
...
- [ ] In `frontend/src/pages/rider/RiderDashboard.tsx`:
...
- [ ] In `backend/src/routes/rides.js`:
...
- [ ] In `frontend/src/pages/delivery/DeliveryDashboard.tsx` & ambulance fleet apps:
...
- [ ] In `backend/src/services/emergencyDispatchService.js`:
...
- [ ] In `frontend/src/pages/clinic/ClinicDashboard.tsx`:
...
- [ ] In `frontend/src/pages/lawyer/LawyerDashboard.tsx`:
  ...
  - Hold demo retainer fee in escrow; auto-release upon consultation sign-off.

### 2.5 Medical Assistant Home Healthcare Flow
- [ ] In `frontend/src/pages/assistant/AssistantDashboard.tsx`:
...
- [x] Register Protobuf / Avro schemas with Confluent Schema Registry contracts.
- [x] Enforce producer idempotence (`enable.idempotence=true`) and strict acknowledgment (`acks=all`).
...
- [x] Implement exponential backoff retry queues (`findmedi.retry.5s` $\to$ `findmedi.retry.30s` $\to$ `findmedi.dlq`).
...
- [x] Create Flink SQL streaming job definitions reading `findmedi.dispatch.booking-events.v1`.
- [x] Compute 1-minute sliding window aggregations for supply vs. demand per H3 cell.
- [x] Emit dynamic surge multiplier output to Apache Pinot sink.

### 4.2 Apache Pinot Real-Time OLAP Tables
- [x] Configure Pinot real-time table `emergency_dispatch_metrics` streaming directly from Kafka.
- [x] Connect SuperAdmin Dashboard cockpit to query Pinot SQL endpoints for sub-50ms operational heatmaps.

### 4.3 OpenSearch Distributed Search Cluster
- [x] Setup OpenSearch indices: `findmedi_providers_v1` and `findmedi_audit_logs_v1`.
...
- [x] Define object storage schema for `findmedi-data-lake`.
- [x] Deploy Apache Hudi DeltaStreamer specifications ingesting raw Kafka telemetry into parquet Merge-on-Read (MoR) tables.
- [x] Configure Apache Hive / Trino schemas for multi-million row historical tax audit queries.
...
- [x] Forward binned GPS batches to Redis and Kafka at $>120,000$ packets/sec.
