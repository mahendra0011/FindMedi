...
router.post('/referrals', protect, async (req, res) => {
  try {
    ...
    const r = await Physiotherapy.create({ 
      ...
      doctorId: req.user._id, doctorName: req.user.name, createdBy: req.user._id 
    });
...
router.put('/referrals/:id/assess', protect, async (req, res) => {
  try {
    const r = await Physiotherapy.findByIdAndUpdate(req.params.id, { initialAssessment: req.body, status: 'In Progress', treatmentPlan: req.body.treatmentPlan }, { new: true });
...
router.put('/referrals/:id/complete', protect, async (req, res) => {
  try {
    const r = await Physiotherapy.findByIdAndUpdate(req.params.id, { status: 'Completed', dischargePlan: req.body }, { new: true });
    if (!r) return res.status(404).json({ message: 'Not found' });
    res.json(r);
  } catch (err) { res.status(400).json({ message: err.message }); }
});
...
router.get('/stats', protect, async (req, res) => {
  const active = await Physiotherapy.countDocuments({ status: { $in: ['Referred','In Progress','Mid Review'] } });
  const completed = await Physiotherapy.countDocuments({ status: 'Completed' });
  const total = await Physiotherapy.countDocuments();
