/**
 * Test card — used on diagnostic test directory pages.
 *
 * Ported from client/src/components/TestCard.jsx.
 * Migration changes:
 *   - framer-motion → motion/react
 *   - Consolidated the 4 duplicate discount branches into a single check
...
export type ProviderType = 'hospital' | 'clinic' | 'lab_technician' | 'phlebotomist' | 'radiographer' | 'sonographer';
...
export interface Test {
  ...
  mode?: string;
  linkedDoctor?: string;
  admissionReq?: boolean;
  homeCollection?: boolean;
  homeCollectionFee?: number;
  reportsOnline?: boolean;
  packageLink?: string;
  doctor?: string;
  quickTest?: boolean;
  ...
  scanType?: string;
  certifiedPhlebotomist?: boolean;
  certifiedSonographer?: boolean;
  onBook?: () => void;
  onUploadRx?: () => void;
  onViewProvider?: () => void;
}
...
function PrescriptionBadge({ required }: { required: boolean }) {
  return required
...
interface TagPillProps {
  ...
  variant?: 'default' | 'success' | 'amber' | 'blue' | 'purple' | 'rose';
}

function TagPill({ icon: Icon, children, variant = 'default' }: TagPillProps) {
...
function ProviderIcon({ type }: { type: ProviderType }) {
  const icons: Record<ProviderType, React.ComponentType<{ className?: string }>> = {
    hospital: Building2,
    clinic: Stethoscope,
...
export default function TestCard({ test, index = 0 }: TestCardProps) {
  const {
  ...
  // Consolidated: all provider types use the same discount branch
    ...
    >
      <div className="p-4 space-y-3 flex-1 flex flex-col">
        ...
        <div className="flex items-center flex-wrap gap-1.5">
          ...
          {(isLabTechnician || isPhlebotomist) && nablAccredited && (
