...
import { isValidFileUrl } from '@/lib/utils';
import { 
  parseTime, subSlotFor, getHourSlots, getSubSlotsForHour, hourBoxFor 
} from '@/lib/timeSlots';
import { CompletedCard } from './TodayAppointmentsSection';
...
export default function AppointmentHistorySection({ appointments = [] }: AppointmentHistorySectionProps) {
  const [calDate, setCalDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(getISTDateString());
  const [viewMode, setViewMode] = useState<'complete' | 'absent' | 'payments'>('complete');
  ...
  const subSlots = useMemo(() => {
    ...
    return present.length ? present : getSubSlotsForHour(hourBoxFor(sorted[0]?.time) || hourSlots[0] || '09:00 AM');
  ...
  // Average completed per active day
  ...
  const busiestDay = useMemo<{ date: string; count: number } | null>(() => {
  ...
  const handleSubSlotClick = (s: string | null) => {
              ...
              {slotAppointments.map((apt, i) => (
                <div key={apt._id || i} id={`history-card-${apt._id}`} className="scroll-mt-4">
                  <CompletedCard 
                    apt={apt} 
                    onViewFile={(url) => {
                      if (!isValidFileUrl(url)) { toast.error('File unavailable. Ask the patient to re-upload it.'); return; }
      ...
      <div className="space-y-4 flex flex-col md:min-h-0">
        <div className="bg-card rounded-2xl border border-border/60 p-4 shadow-sm shrink-0">
          ...
          <div className="grid grid-cols-3 gap-2">
            <StatusTile icon={Clock} label="Today" value={completedStats.today} />
            <StatusTile icon={CalendarDays} label="Last 7D" value={completedStats.last7} />
            <StatusTile icon={CalendarCheck2} label="This Month" value={completedStats.month} tone="success" />
            {showMore && (
              <>
                <StatusTile icon={History} label="Last 6M" value={completedStats.last6m} tone="warning" />
                <StatusTile icon={Award} label="This Year" value={completedStats.year} tone="warning" />
                <StatusTile icon={BarChart3} label="All" value={completedStats.all} tone="warning" />
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <StatusTile icon={Clock} label="Today" value={absentStats.today} tone="danger" />
            <StatusTile icon={CalendarDays} label="Last 7D" value={absentStats.last7} tone="danger" />
            <StatusTile icon={CalendarCheck2} label="This Month" value={absentStats.month} tone="danger" />
            {showMore && (
              <>
                <StatusTile icon={History} label="Last 6M" value={absentStats.last6m} tone="danger" />
                <StatusTile icon={Award} label="This Year" value={absentStats.year} tone="danger" />
                <StatusTile icon={BarChart3} label="All" value={absentStats.all} tone="danger" />
              </>
            )}
          </div>
