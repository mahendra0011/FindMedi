...
import { IndianRupee, TrendingUp, Banknote } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { useDeliveryProfile, useMyDeliveries } from '@/features/delivery';
...
export default function DeliveryEarningsPage() {
  ...
  const filteredDeliveries = history.filter((d) => {
    const date = new Date((d.deliveredAt ?? d.createdAt ?? '') as string);
  ...
  const bank = (profile as unknown as { bankDetails?: { accountNo?: string; ifsc?: string; holderName?: string; upiId?: string } })?.bankDetails;
        ...
        {bank?.accountNo ? (
          <div className="space-y-2 text-sm">
            <div className="flex justify-between py-2 border-b border-border/40">
              ...
              <span className="font-medium text-foreground">••••{bank.accountNo.slice(-4)}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-border/40">
              <span className="text-muted-foreground">IFSC Code</span>
              <span className="font-medium text-foreground">{bank.ifsc}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-border/40">
              <span className="text-muted-foreground">Account Holder</span>
              <span className="font-medium text-foreground">{bank.holderName}</span>
            </div>
            {bank.upiId && (
              <div className="flex justify-between py-2">
                <span className="text-muted-foreground">UPI ID</span>
                <span className="font-medium text-foreground">{bank.upiId}</span>
              </div>
            )}
          </div>
        ) : (
          ...
          {filteredDeliveries.slice(0, 10).map((d) => (
            <div key={d._id} className="bg-card rounded-lg border p-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                >
                  <IndianRupee
                    className={`w-4 h-4 ${d.status === 'Delivered' ? 'text-emerald-600' : 'text-destructive'}`}
                  />
                </div>
                <div>
                  <p className="text-sm font-medium text-foreground">Order #{d.orderId ?? d._id.slice(-6)}</p>
