...
const featuredCounsellors = [
  { name: "Aisha Mehra", image: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=400&h=400&fit=crop&crop=face&auto=format", role: "Anxiety and Stress Management", rating: "4.9", reviews: "128", bio: "Licensed psychologist helping students manage anxiety, panic, exam stress, and emotional overwhelm with practical coping plans.", tags: ["Anxiety", "Stress", "Student Pressure"], exp: "8 years exp", location: "Mumbai", language: "English / Hindi", price: "₹1,799" },
  { name: "Arjun Sen", image: "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400&h=400&fit=crop&crop=face&auto=format", role: "Sleep, Burnout and Workload Balance", rating: "4.8", reviews: "119", bio: "Professional psychologist helping students and early professionals with burnout, sleep routines, stress recovery, and boundaries.", tags: ["Sleep", "Burnout", "Career Stress"], exp: "9 years exp", location: "Kolkata", language: "English / Bengali / Hindi", price: "₹1,499" },
  { name: "Neha Iyer", image: "https://images.unsplash.com/photo-1594824476967-48c8b964273f?w=400&h=400&fit=crop&crop=face&auto=format", role: "Depression and Mood Support", rating: "4.8", reviews: "142", bio: "Professional counsellor supporting low mood, loneliness, grief, emotional numbness, and therapy progress tracking.", tags: ["Depression", "Loneliness", "General"], exp: "10 years exp", location: "Chennai", language: "English / Tamil", price: "₹1,649" },
  { name: "Priya Nair", image: "https://images.unsplash.com/photo-1587614382344-4ecb093b79b2?w=400&h=400&fit=crop&crop=face&auto=format", role: "Trauma Support and Grounding", rating: "4.9", reviews: "166", bio: "Trauma-informed therapist helping clients with grounding, safety planning, triggers, PTSD symptoms, and emotional regulation.", tags: ["Trauma Support", "PTSD", "Anxiety"], exp: "12 years exp", location: "Kochi", language: "English / Malayalam / Hindi", price: "₹1,799" },
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-5xl font-bold mb-3">
            Our <span className="gradient-text">counsellors</span>
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto">Find someone you click with — verified professionals ready to support you.</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-10">
          <div className="relative">
            ...
            <Input
              ...
              className="h-11 rounded-xl bg-background border-border pl-11 text-sm text-foreground placeholder:text-muted-foreground focus-visible:border-primary/50 focus-visible:ring-primary/20"
            />
          </div>
            ...
            {allCategories.slice(0, 12).map(item => (
              <button key={item} onClick={() => setCategory(item)}
                className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition-all ${
                  category === item ? "bg-primary text-primary-foreground shadow-lg shadow-primary/25" : "bg-muted text-muted-foreground hover:text-foreground"
                }`}
              >
                {item}
              </button>
          ...
          <p className="text-center text-xs text-muted-foreground mt-3">{filtered.length} of {featuredCounsellors.length} counsellors found</p>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {visibleCounsellors.map(c => (
              <div key={c.name} className="group bg-card border border-border/60 rounded-2xl overflow-hidden hover:border-primary/30 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                ...
                <div className="p-4 space-y-2.5">
                  <div className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className="text-muted-foreground">{c.exp}</span>
                  </div>
                  <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{c.bio}</p>
                  <div className="flex flex-wrap gap-1">
                    {c.tags.slice(0, 3).map(t => (
                      <span key={t} className="px-2 py-0.5 bg-primary/10 text-primary rounded text-[10px] font-medium border border-primary/20">{t}</span>
                    ))}
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-border/50">
                    <div>
                      <p className="text-[10px] text-muted-foreground">Starts at</p>
                      <p className="text-base font-bold text-foreground">{c.price}<span className="text-[10px] font-normal text-muted-foreground ml-1">/ pkg</span></p>
                    </div>
                    <Button onClick={() => navigate("/mind/counselling")} className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl px-4 h-9 text-xs font-medium shadow-lg shadow-primary/25">
        ...
        <div className="text-center mt-10">
          <Button
            ...
            className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-8 py-6 text-sm font-bold shadow-lg shadow-primary/25 transition-all duration-300 hover:scale-105 group"
          >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        ...
        <div>
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-base md:text-lg max-w-xl mx-auto">From first message to first session in three simple steps.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {steps.map((step, i) => (
              <div key={step.title} className="relative z-10">
                <div className="bg-card border border-border/60 rounded-[28px] p-8 text-center hover:border-primary/40 hover:shadow-xl transition-all duration-500 hover:-translate-y-2 h-full">
                  ...
                  <div className="relative w-20 h-20 mx-auto mb-6">
                    ...
                    <div className="relative w-20 h-20 bg-gradient-to-br from-primary/15 to-secondary/15 rounded-full flex items-center justify-center border border-primary/25">
                  ...
                  <div className="mt-6 h-1 w-12 bg-gradient-to-r from-primary to-secondary rounded-full mx-auto opacity-60" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-700 text-xs font-medium mb-4">
            ...
            <p className="text-muted-foreground text-base md:text-lg max-w-xl mx-auto">Hear from people who found the support they needed.</p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="grid md:grid-cols-3 gap-6 relative">
              {stories.map(([quote, name, meta], i) => (
                <div key={name} className="relative group bg-card border border-border/60 rounded-[24px] p-7 hover:border-primary/30 hover:shadow-lg transition-all duration-500 hover:-translate-y-1">
        ...
        <div>
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-base md:text-lg max-w-xl mx-auto">Everything you need to know before your first session.</p>
          </div>

          <div className="max-w-3xl mx-auto space-y-4">
            {faqs.map((f, i) => (
              <details key={f.q} className="group bg-card border border-border/60 rounded-2xl overflow-hidden transition-all duration-300 hover:border-primary/30 open:border-primary/30 open:shadow-lg">
                <summary className="cursor-pointer px-6 py-5 text-sm font-medium flex items-center justify-between list-none">
                  <span className="flex items-center gap-3">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-primary/15 text-primary text-xs font-bold">{String(i + 1).padStart(2, "0")}</span>
                    <span className="text-foreground group-hover:text-foreground transition-colors">{f.q}</span>
                  </span>
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground transition-all duration-300 group-open:bg-primary/20 group-open:text-primary group-open:rotate-45">
                ...
                <div className="px-6 pb-5 pt-0">
                  <div className="h-px bg-gradient-to-r from-primary/30 via-secondary/30 to-transparent mb-4" />
        ...
        <div className="bg-gradient-to-br from-red-500/10 to-rose-500/5 border border-red-500/20 rounded-2xl p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-4 md:items-center">
            <div className="flex-1">
              <h3 className="text-lg font-bold mb-1 text-foreground">In crisis? You are not alone.</h3>
              <p className="text-sm text-muted-foreground">If you or someone you know is in immediate distress, reach out to a 24/7 helpline. Therapy is ongoing care and emergencies need urgent support.</p>
            </div>
            <Button onClick={() => navigate("/mind/resources")} className="bg-red-500 hover:bg-red-400 text-white rounded-xl px-6 h-10 font-medium shrink-0">
