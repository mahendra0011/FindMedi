/**
 * Diagnostic center card — used on diagnostic center directory.
 *
 * Ported from client/src/components/DiagnosticCenterCard.jsx.
 * Migration changes:
 *   - framer-motion → motion/react
 *   - react-router-dom useNavigate → next/navigation useRouter
 *   - TypeScript prop types (replaces dynamic prop destructuring)
 */
'use client';
...
interface DiagnosticCenterCardProps {
  clinic: DiagnosticCenter;
  index?: number;
}
...
const TECHNICIAN_DATA: Record<string, TechnicianInfo> = {
  'Pathology Lab': {
    name: 'Ramesh Kumar',
    role: 'Lab Technician',
...
function getTechFallback(type: string): TechnicianInfo {
  return TECHNICIAN_DATA[type] ?? TECHNICIAN_DATA['Diagnostic Center']!;
}

const STAR_SIZE = 'w-3 h-3';
...
interface TagBadgeProps {
  ...
  variant?: 'primary' | 'success' | 'amber' | 'blue' | 'purple';
}

function TagBadge({ icon: Icon, children, variant = 'primary' }: TagBadgeProps) {
...
export default function DiagnosticCenterCard({ clinic, index = 0 }: DiagnosticCenterCardProps) {
  ...
  const tech: TechnicianInfo = technicianName
    ? {
        name: technicianName,
        role: technicianRole || getTechFallback(type).role,
        qualifications: technicianQualification ? technicianQualification.split(', ') : getTechFallback(type).qualifications,
        experience: technicianExperience || getTechFallback(type).experience,
        regNo: '',
      }
    : getTechFallback(type);
  const techInitials = tech.name.split(' ').map(n => n[0]).join('').slice(0, 2);
  const labInitials = name?.split(' ').map(n => n[0]).join('').slice(0, 2) || 'TC';
  ...
  return (
    <motion.div
      ...
      className="group bg-card rounded-2xl border border-border/50 overflow-hidden hover:shadow-xl hover:shadow-primary/10 hover:border-primary/30 transition-all duration-300 flex flex-col h-full"
    >
      {/* ─── Cover ─── */}
      <div className={cn(
      ...
      <div className="px-4 pt-3 pb-4 space-y-2.5 flex-1 flex flex-col">
        {/* Identity: Lab Name + Technician + Designation */}
        ...
        <div className="grid grid-cols-2 gap-2 pt-1 mt-auto">
          <Button
            ...
            onClick={() => navigate.push(`/book-test/${_id}`)}
          >
          ...
          <Button
            variant="outline"
            className="gap-1.5 rounded-xl h-10 text-xs font-semibold"
            onClick={() => navigate.push(`/technician/${_id}`)}
          >
