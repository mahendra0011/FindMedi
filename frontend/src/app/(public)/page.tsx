/* eslint-disable @next/next/no-img-element -- legacy remote imagery uses plain <img> */
'use client';

/**
 * Home page — rich legacy homepage ported from client/src/pages/Home.jsx.
 *
 * Client component (animations + widgets need it). Public chrome (navbar /
 * footer) comes from frontend/src/app/(public)/layout.tsx, so it is not
 * duplicated here.
 */
...
const doctorImage =
  'https://alliedsoftech89.wordpress.com/wp-content/uploads/2013/06/medical-doctor-jobs-in-china-expat-jobs-in-china.jpg';
const doctorImage2 =
  'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSjPqcWATF_Dr7kcC-DSSbsfzCtcFZDdeI-pQ&s';
...
const statsData: { label: string; value: number; suffix: string; icon: IconType }[] = [
...
const services: { icon: IconType; name: string; price: string }[] = [
...
const specialties: { icon: IconType; name: string; color: string; count: string }[] = [
...
const whyChooseUs: { icon: IconType; title: string; desc: string; color: string; iconColor: string }[] = [
  {
    icon: Building2,
    title: 'All-in-One Connected Ecosystem',
    desc: 'From instant OPD token registration to intensive IPD bedside charting and complex OT scheduling, manage every single clinical and administrative workflow within a single unified platform.',
    color: 'from-emerald-500/10 to-emerald-500/5',
    iconColor: 'text-emerald-600',
  },
  {
    icon: Zap,
    title: 'Smart Front-Desk Automation',
    desc: 'Eliminate long waiting lines and chaotic waiting rooms with automated UHID generation, dynamic department-wise token systems, live queue tracking, and real-time estimated wait-time algorithms.',
    color: 'from-blue-500/10 to-blue-500/5',
    iconColor: 'text-blue-600',
  },
  {
    icon: ClipboardList,
    title: 'Integrated Diagnostics & Care',
    desc: 'Seamlessly bridge the gap between doctors and support wings with connected 29+ Lab test catalogs, digital Radiology viewers, allergy-checked Pharmacy dispensing, and real-time Blood Bank registries.',
  ...
  },
  {
    ...
    desc: "Streamline your hospital's cash flow with automated invoice generation (INV-0001), multi-mode payment tracking (UPI/Card/Cash), partial-payment workflows, and integrated insurance pre-authorization.",
  ...
  },
  {
    ...
    desc: 'Optimize backend resources with bulk staff attendance tracking, automated shift payrolls, predictive inventory reorder alerts, and automated housekeeping tasks triggered instantly upon patient discharge.',
  ...
  },
  {
    ...
    desc: 'Protect sensitive health records with strict role-scoped access control (Superadmin, Admin, Doctor, Patient), secure Cloudinary-backed medical document storage, and 100% data confidentiality.',
...
const featureSections: {
...
}[] = [
  {
    ...
    subtitle: 'End-to-end digital experience from registration to discharge',
    ...
    items: [
      {
        ...
        desc: 'New patient onboarding, UHID generation, demographic capture, and insurance verification in seconds',
      },
      {
        icon: CalendarCheck,
        title: 'OPD Registration',
        desc: 'Outpatient registration with automated token generation, appointment scheduling, and live queue management',
      },
      {
        icon: HeartPulse,
        title: 'Doctor Consultation',
        desc: 'Comprehensive OPD & IPD consultations with certified specialists, e-prescriptions, and follow-up tracking',
      },
      {
        icon: FileText,
        title: 'Medical Records',
        desc: 'Unified digital health record with prescription history, lab results, discharge summaries, and document uploads',
      },
      {
        icon: BedDouble,
        title: 'IPD Management',
        desc: 'Bed allocation, ward management, nursing charts, diet orders, and discharge planning with auto-housekeeping',
      },
    ],
  },
  {
    ...
    subtitle: 'Rapid response systems for life-saving decisions',
    ...
    items: [
      {
        ...
        desc: 'P1-P5 severity triage, MLC registration, real-time vitals tracking, and instant critical care alerts',
      },
      {
        icon: Scissors,
        title: 'Operation Theatre',
        desc: 'OT scheduling, pre-op checklists, instrument sterilization logs, surgeon assignment, and post-op recovery tracking',
      },
      {
        icon: Droplet,
        title: 'Blood Bank',
        desc: 'Donor management, blood unit screening, crossmatch validation, issue tracking, and transfusion reaction logging',
      },
      {
        icon: Dna,
        title: 'Physiotherapy',
        desc: 'Referral-based therapy with exercise library, session tracking, pain scale monitoring, and home exercise plans',
      },
    ],
  },
  {
    ...
    subtitle: 'Integrated clinical workflows connecting labs, pharmacy, and nutrition',
    ...
    items: [
      {
        icon: TestTube,
        title: 'Lab & Radiology',
        desc: '29+ test catalog with auto-abnormal flags, sample tracking, pathologist verification, and radiology image viewer',
      },
      {
        icon: Package,
        title: 'Inventory & Pharmacy',
        desc: 'Real-time stock tracking, expiry alerts, allergy-checked dispensing, drug interaction warnings, and vendor management',
      },
      {
        icon: UtensilsCrossed,
        title: 'Diet & Kitchen',
        desc: 'Patient-specific diet plans, allergy-safe meal rotation, kitchen task assignment, and meal delivery confirmation',
      },
    ],
  },
  {
    ...
    subtitle: 'Holistic care extending beyond treatment',
    ...
    items: [
      {
        icon: Smile,
        title: 'Mental Health',
        desc: 'Confidential assessments, risk scoring, therapy goal tracking, medication management, and family involvement logging',
      },
      {
        icon: Sofa,
        title: 'Housekeeping',
        desc: 'Auto-triggered cleaning on discharge, task assignment, infection control checklists, and bed status verification',
      },
      {
        icon: Bell,
        title: 'Smart Notifications',
        desc: 'Real-time alerts for appointments, lab results, billing reminders, emergency alerts, and prescription refills',
      },
    ],
  },
  {
    ...
    subtitle: 'Streamline staff management and administrative workflows',
    ...
    items: [
      {
        ...
        desc: 'Complete lifecycle from onboarding to payroll — attendance, shift scheduling, overtime, and performance reports',
      },
      {
        icon: Upload,
        title: 'File Upload & Reports',
        desc: 'Bulk import/export of patient data and lab reports, automated PDF generation for medical documents',
      },
    ],
  },
  {
    ...
    subtitle: 'Revenue cycle management with data-driven insights',
    ...
    items: [
      {
        ...
        desc: 'Automated invoice generation (INV-0001), UPI/Card/Cash payments, partial payments, insurance pre-auth and claim filing',
      },
      {
        icon: PieChart,
        title: 'Reports & Analytics',
        desc: 'Exportable reports, revenue trends, department-wise performance metrics, demographic analysis, and compliance dashboards',
      },
    ],
  },
];

const testimonials: { name: string; role: string; image: string; content: string; rating: number }[] = [
  {
    name: 'Sarah Johnson',
    role: 'Patient',
  ...
  },
  {
    ...
    content:
      'Outstanding service! The booking process was smooth and the doctor was incredibly knowledgeable. Highly recommend FindMedi.',
    rating: 5,
  },
  {
    name: 'Emily Williams',
    ...
    content:
      'From scheduling to follow-up, every step was handled with professionalism. The team truly cares about patient well-being.',
    rating: 5,
  },
];

const fallbackDoctors: HomeDoctor[] = [
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const wrapped = data as unknown as { doctors?: unknown; data?: unknown };
        const list: unknown = wrapped?.doctors ?? wrapped?.data ?? data;
        const arr = Array.isArray(list) ? (list as HomeDoctor[]) : [];
        if (!cancelled) setDoctorsList(arr.slice(0, 6));
      } catch {
  ...
  useEffect(() => {
    try {
      const city =
        window.localStorage.getItem('findmedi_city') || window.localStorage.getItem('mediCore_city') || '';
  ...
  const dashboardLabel =
    user?.role === 'hospital_admin' ? 'Admin Dashboard' : user?.role === 'doctor' ? 'Doctor Dashboard' : 'User Dashboard';
  const primaryActionPath = user ? '/dashboard' : '/signup';
  const hospitalsHref = selectedCity ? `/hospitals?city=${encodeURIComponent(selectedCity)}` : '/hospitals';

  const handleCtaPointerMove = (event: ReactMouseEvent<HTMLElement>) => {
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                <Button
                  size="lg"
                  className="gap-2 text-base px-8 h-14 shadow-xl shadow-primary/25"
            ...
            >
              ...
              <div className="relative overflow-hidden rounded-[28px] shadow-2xl">
                <img src={heroImage} alt="FindMedi Healthcare" className="w-full h-[400px] object-cover" />
      ...
      <section className="py-12 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Button variant="outline" className="h-16 text-lg gap-3 py-4" onClick={() => router.push(hospitalsHref)}>
              <Search className="w-5 h-5" /> Find by Speciality
            </Button>
            <Button variant="outline" className="h-16 text-lg gap-3 py-4" onClick={() => router.push(hospitalsHref)}>
              <MapPin className="w-5 h-5" /> {selectedCity || 'Find by Location'}
            </Button>
            <Button
              variant="outline"
              className="h-16 text-lg gap-3 py-4"
              onClick={() => router.push(primaryActionPath)}
            >
        ...
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" variants={fadeUp} className="text-center mb-14">
            ...
            <p className="text-muted-foreground mt-3 max-w-2xl mx-auto">
              From consultation to discharge, every aspect of hospital management is covered with our 20+ integrated
              digital modules
            </p>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {specialties.map((spec, i) => (
              ...
              >
                <div
                  className={`w-14 h-14 rounded-2xl ${spec.color} flex items-center justify-center mx-auto mb-4 transition-transform group-hover:scale-110`}
                >
        ...
        >
          <div className="rounded-[32px] px-4 py-10 sm:px-8 lg:px-10">
            <motion.div initial="hidden" whileInView="visible" variants={fadeUp} className="text-center mb-14">
              ...
              <p className="text-muted-foreground mt-3 max-w-2xl mx-auto">
                We combine cutting-edge automation with interconnected modules to deliver a secure, efficient, and
                next-generation healthcare management experience.
              </p>
            </motion.div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="relative rounded-3xl overflow-hidden shadow-2xl">
                <img src={doctorImage} alt="Our Doctors" className="w-full h-[500px] object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
              </div>
              <motion.div
                animate={{ y: [-10, 10, -10] }}
            ...
            <motion.div initial="hidden" whileInView="visible" variants={fadeUp}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {(doctorsList.length > 0 ? doctorsList : fallbackDoctors).map((doc, i) => (
                  <motion.div
                    key={doc.name ?? i}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {doc.available && <span className="w-2 h-2 rounded-full bg-green-500" />}
                      </div>
              ...
              <div className="mt-8">
                <Button size="lg" variant="outline" className="gap-2" onClick={() => router.push('/doctors')}>
      ...
      <section className="py-20 px-4 sm:px-6 bg-gradient-to-b from-muted/10 via-background to-muted/10">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[0.75fr_1.25fr] gap-10 lg:gap-14 items-center">
          <motion.div initial="hidden" whileInView="visible" variants={fadeUp}>
            ...
            <Button className="mt-8 gap-2" onClick={() => router.push('/diagnostic-centers')}>
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="text-white/80 text-lg max-w-xl mx-auto mb-8">
              Join 20,000+ patients who trust FindMedi for their healthcare needs. Book appointments, manage records,
              and access 20+ integrated hospital modules — all in one place.
            </p>
            <Button
              size="lg"
              className="group gap-2 text-base px-10 h-12 bg-white text-primary hover:bg-white/90 shadow-xl"
              onClick={() => router.push(primaryActionPath)}
            >
              {user ? `Open ${dashboardLabel}` : 'Book Appointment'}{' '}
