/**
 * Manage Reviews — ported from client/src/pages/admin/AdminReviews.jsx (Phase 4).
...
export default function ReviewsPage() {
  ...
  const { data: reviewsData, isLoading: loading } = useQuery({
    ...
    queryFn: async (): Promise<AdminReview[]> => {
      ...
      return (Array.isArray(res) ? res : []) as unknown as AdminReview[];
    },
    staleTime: 30_000,
  });
  ...
  const handleDelete = (id: string) => {
    ...
    deleteMut.mutate(id, { onError: () => toast.error('Failed to delete review') });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-2xl border border-border/60 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-muted-foreground">Low Rating (1-2)</p>
            <p className="text-xl font-bold">{reviews.filter((r) => r.rating <= 2).length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Search reviews by doctor or patient name..."
          className="pl-10"
        />
      </div>
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {rv.rating <= 2 && (
                      <span className="ml-2 px-2 py-0.5 rounded-full text-xs bg-destructive/10 text-destructive font-semibold">
                        Flagged
                      </span>
                    )}
                  </div>
